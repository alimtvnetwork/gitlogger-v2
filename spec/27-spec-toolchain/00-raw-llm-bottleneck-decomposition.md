...
**Version:** 1.11.0
**Updated:** 2026-05-11 (Sess-80 B-33 — closes the §27 walker-cost asymmetry on `00-tier1-bundle.md` v1.2.0 → v1.3.0: per-file Walker-cost (KB) column added to that file's Tier-1 table (Σ ~419 KB across §00 ~83 KB / trace-map ~50 KB / §97 ~101 KB / §99 ~185 KB — §27 is the largest in-scope cohort by tier-1 KB) + new "Walker-cost reflexivity (load-budget pin)" section with 4 pre-budget recipes + reflexive drift contract (special §99 cadence: every 5th `-stamp-bump` OR any 10 KB bucket cross). Cites mirror-septet anchor closure (Sess-79 B-27-§26) — 7 of 7 in-scope cohorts now carry the column. Per-criterion table C6 row **16 → 17** band-anchor advance (cited mechanism: walker-cost column on tier-1 bundle itself + mirror-septet anchor closure citation as 4th + 5th independent levers — total 5 cited mechanisms for C6 now). Σ R-band 110 → **111**. §27 still sole Raw-LLM cohort floor at R 111 (§24 + §28 + §22 + §23 carried at 119; §25 at 120; §26 at 120 post Sess-79 B-27-§26). Aggregate Raw-LLM Σ 820 → **821/840** (97.7/100). Same-PR banner-triple lockstep (§00 / §98 / §99 / tier-1-bundle).)
**Updated-prev:** 2026-05-11 (Sess-74 B-28-stub-sweep — closes B-28-stub-sweep: per-criterion table C3 row 17 → 18 (band-anchor reached); Σ R-band 109 → 110. §27 still sole Raw-LLM cohort floor at R 110.)
**Updated-prev2:** 2026-05-11 (Sess-73 B-28 — C3 row 16 → 17; AC-T-41 minted; Σ R-band 108 → 109.)
...
| **C3 Testability** | **18/20** | Band-anchor | ~38 KB (AC-T-39 5-clause structural contract + 26-active-slot ceiling attestation tail + AC-T-41 closed 5-token enum body + worked-example `comm -23` verifier sketches for both ACs + 53 on-disk `**Test pair:** N/A — <axis-class>` stub lines @ ~80 B = ~4 KB) | ✓ closed by B-19 + B-19b + B-28 + B-28-stub-sweep (Sess-69 + Sess-73 + Sess-74) | AC-T-39 5-clause structural contract + 26/26 active-gate-slot ceiling reached on disk + AC-T-41 5-clause drift contract + closed 5-token axis-class enum `{advisory, generator, scaffold, audit-aid, convention}` for 53 non-gate slots + R-4 reverse-coverage invariant **satisfied at 5/5** (advisory 22, generator 11, audit-aid 9, scaffold 7, convention 4 — Sess-74 inserter walk) + worked-example `comm -23` verifier sketch (returns 0 missing slots at ceiling for both ACs — verified on disk Sess-74) — together cover **79/79** slot-level testability surface with 3 of 4 AC-T-41 legs structural rather than reviewer-attestation-only | Ceiling 19-20 deferred to (a) gate #42 clause-6 `active-gate-test-pair-presence` extension making AC-T-39 verifier loop hard CI fail (lift C3 18 → 19, `-impl`), (b) gate #42 clause-7 `non-gate-axis-class-presence` extension making AC-T-41 verifier loop hard CI fail (lift C3 19 → 20 ceiling, `-impl`). |
| **C4 Consistency** | **16/20** | Band-anchor | ~21 KB (AC-T-37 four-way parity body + callers parity 5th-dimension amendment + AC-T-30/-31/-36 navigation-quintet + slot 66 schema body + `00-mechanism-citation-index.md` §1+§2 reverse index) | ✓ closed by B-26 (Sess-72) | AC-T-37 four-way parity contract + B-22 carriers exclusion clause + B-26 callers reverse-index clause (5th-dimension callers parity) + `00-mechanism-citation-index.md` v1.0.0 11-row reverse index inverting cross-cohort citation graph (audit-followable in 0 hops vs 7-cohort grep) + drift-contract clauses D-1/D-2/R-3 — reviewer-attestation today; long-tail `-impl` `build-mechanism-citation-index.py` out of scope per `mem://constraints/no-implementation-suggestions` | Ceiling 18-20 deferred to (a) **B-20-impl** (script + workflow row + AC-T-40 mint + banner-triple bump 26 → 27 → C4 16 → 17 band-anchor+1), (b) **B-16-impl** (mass-edit across 28 active-gate slot files inserting `navigation_quintet` frontmatter row → C4 17 → 18 band-anchor), (c) **B-22-impl** (file moves into `_carriers/` + slot-61 gate #39 link-resolution extension + §00 Inventory partition table + glob sweep → C4 18 → 19), (d) long-tail `build-mechanism-citation-index.py` promoting B-26 callers parity from reviewer-attestation to mechanical enforcement → C4 19 → 20 ceiling. |
...
| **C6 Friction** | **17/20** | Band-anchor (advanced Sess-80) | ~13 KB (Tier-1 bundle Tier-2 slot-index lookup table + drift-contract clause + this scoreboard's own walker-cost column from B-27 + Sess-80 B-33 walker-cost column on `00-tier1-bundle.md` Tier-1 table itself + 4 pre-budget recipes + mirror-septet anchor closure citation) | ✓ advanced by B-33 (Sess-80) | 5 cited mechanisms now: (1) `00-tier1-bundle.md` v1.2.0 Tier-2 slot-index lookup table + drift-contract clause + reverse-lookup + active-count parity note (B-21/B-27 Sess-71); (2) AC-T-38 tier-promotion drift contract; (3) this scoreboard's per-criterion **walker-cost (KB)** column (Sess-71 B-27); (4) NEW Sess-80 B-33 walker-cost column on `00-tier1-bundle.md` v1.3.0 Tier-1 table itself (closes the §27 walker-cost asymmetry — §27 was the originating cohort but did not carry the column on its own tier-1 manifest until now); (5) NEW Sess-80 B-33 mirror-septet anchor closure citation (Sess-79 B-27-§26 closed the cross-cohort anchor — 7 of 7 in-scope cohorts cite walker-cost reflexivity, and §27's tier-1 bundle now carries the closure citation as a self-enforcing cross-cohort consistency lever) | Ceiling 18 deferred to a future cross-cohort gate that mechanically verifies the carriers-exclusion glob across all 7 spec cohorts (long-tail `-impl`). Ceiling 19/20 deferred to a per-criterion walker-cost drift gate extending gate #42 clause-5 to recompute KB-costs against on-disk file sizes (long-tail `-impl`); §27 §99 special cadence (every 5th `-stamp-bump` OR any 10 KB bucket cross) makes this the most acute gate-extension target across all 7 cohorts. |
| **Σ R-band** | **111/120** | — | ~92 KB total (sum of cited-mechanism payloads; +4 KB from Sess-80 B-33 walker-cost reflexivity additions to `00-tier1-bundle.md`) | §27 sole Raw-LLM cohort floor at R 111; §24 + §28 + §22 + §23 at R 119; §25 at 120; §26 at 120 post Sess-79 B-27-§26; §27 trails the next-lowest cohort by **8 points** | — | If C4 lands B-20-impl + B-16-impl + B-22-impl: projected R 111 → ~117 (C4 16 → 19). C6 → 18 ceiling deferred to long-tail cross-cohort `-impl` carriers-exclusion gate. C3 → 19 / 20 deferred to long-tail clause-6 / clause-7 extensions of gate #42. Aggregate Raw-LLM Σ projection: 821 + cross-cohort `-impl` deliveries → ~840 ceiling. |
