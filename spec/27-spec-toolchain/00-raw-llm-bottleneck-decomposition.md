...
**Version:** 1.9.0
**Updated:** 2026-05-11 (Sess-73 B-28 — closes B-28: per-criterion table C3 row 16 → 17 (band-anchor reached); cited mechanism extended to include AC-T-41 (closed 5-token testability axis-class enum `{advisory, generator, scaffold, audit-aid, convention}` for 53 non-gate slots) + worked-example `comm -23` verifier + R-4 reverse-coverage invariant; §27 testability quintet now complete (AC-T-39 + AC-T-41 = 79/79 slot coverage at AC ceiling). Σ R-band 108 → 109. §27 no longer ties §25 — §25 becomes sole Raw-LLM cohort floor (R 108); §27 leads §25 by 1. B-28 status "open" → "closed" (spec-only half; per-slot stub-insertion deferred). Aggregate-projection paragraph rewritten — only cross-cohort B-27 mirrors for §24 + §25 + per-slot stub-insertion sweep remain spec-only.)
**Updated-prev:** 2026-05-11 (Sess-72 B-26 — C4 row 15 → 16; cross-cohort reverse index `00-mechanism-citation-index.md` v1.0.0 + AC-T-37 callers parity 5th-dimension; Σ R-band 107 → 108.)
