...
| **Σ R-band** | **108/120** | — | ~78 KB total (sum of cited-mechanism payloads; +3 KB from B-26 reverse index added to C4 cited-mechanism payload) | joint floor (cohort) — ties §25 (R 108); §27 no longer sole floor | — | If B-28 lands: projected 108 → **~109** (C3 +1 via B-28 at band-anchor 17). If B-20-impl + B-16-impl + B-22-impl all land: projected ~109 → **~115** (lifts C4 to 19). Ceiling 120 deferred to long-tail cross-cohort `-impl` candidates (`build-mechanism-citation-index.py` from B-26 long-tail + walker-cost-drift gate from B-27 long-tail + per-slot frontmatter uniformity gate + AC-T-39 ceiling extension to 53 advisory slots). |
