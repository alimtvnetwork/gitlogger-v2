...
**Version:** 1.8.0
**Updated:** 2026-05-11 (Sess-72 B-26 — closes B-26: per-criterion table C4 row 15 → 16 (band-anchor reached); cited mechanism extended to include `00-mechanism-citation-index.md` v1.0.0 (cross-cohort reverse index of `**Mechanically enforced by:**` callers) + AC-T-37 callers parity amendment (5th-dimension extension of four-way parity invariant). Σ R-band 107 → 108. §27 cohort no longer Raw-LLM sole floor — ties §25 (R 108). B-26 status "open" → "closed". Aggregate-projection paragraph rewritten (only B-28 remains spec-only; cross-cohort B-27 mirrors for §24 + §25 now joint floor with §27).)
**Updated-prev:** 2026-05-11 (Sess-71 B-27 — walker-cost (KB) column appended to per-criterion scoreboard; C6 15 → 16 band-anchor; Σ R-band 106 → 107; B-27 closed.)
