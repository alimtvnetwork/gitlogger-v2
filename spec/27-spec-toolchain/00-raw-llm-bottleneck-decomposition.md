...
| **Σ R-band** | **110/120** | — | ~88 KB total (sum of cited-mechanism payloads; +5 KB from 53 on-disk AC-T-41 stub lines added to C3 cited-mechanism payload) | §27 sole Raw-LLM cohort floor at R 110; §24 R 114; §25 R 115; §27 trails §25 by 5 and §24 by 4 | — | If B-27-§24 walker-cost mirror lands (spec-only): projected 110 → ~111 (§24 C6 carried; §27 still floor). If B-20-impl + B-16-impl + B-22-impl all land: projected ~110 → **~116** (C4 16 → 19). Ceiling 120 deferred to long-tail cross-cohort `-impl` candidates (`build-mechanism-citation-index.py` from B-26 + walker-cost-drift gate from B-27 + clause-6 / clause-7 extensions to gate #42 from AC-T-39 / AC-T-41 + per-slot frontmatter uniformity gate from B-20-impl). |
