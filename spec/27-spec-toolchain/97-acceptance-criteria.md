...
**Version:** 2.24.0
**Updated:** 2026-05-11 (Sess-72 B-26 — AC-T-37 amended with **Callers reverse-index clause**: every `**Mechanically enforced by:**` paragraph across the 7 in-scope cohorts naming a §27 active-gate slot MUST be reflected as a row in `00-mechanism-citation-index.md` §1 within ≤ 1 session drift; extends the four-way parity invariant with a fifth dimension — **callers parity** (index row count, minus duplicate `(cohort, AC, slot)` triples, MUST equal `grep -rEn '\*\*Mechanically enforced by:\*\*' spec/22..28/`). Authored alongside `00-mechanism-citation-index.md` v1.0.0 (B-26 spec doc with 11-row reverse index + 8-row gate-multiplicity table + walker-pin carve-out + drift-contract clauses D-1/D-2/R-3) — amendment text quoted from that file's §"Drift contract". AC count carried at **39** (AC-T-37 body amendment only — Lesson #36 link-don't-restate, citing the index by reference rather than enumerating per-cohort callers inline). Lifts §27 R-band C4 (Consistency) **15 → 16** (band-anchor reached; cited mechanism: callers parity inverts the citation graph from one-way per-cohort grep to bidirectional per-slot index, audit-followable in 0 hops vs 7-cohort grep). Long-tail `-impl` candidate `build-mechanism-citation-index.py` (proposed slot 67 / gate #45) explicitly out of scope per `mem://constraints/no-implementation-suggestions`.)
**Version-prev:** 2.23.0
