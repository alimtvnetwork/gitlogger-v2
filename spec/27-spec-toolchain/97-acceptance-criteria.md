...
- **Mechanically enforced by:** `linter-scripts/check-gate-ledger-vs-workflow.py` I-1 EXISTS / I-2 WIRED / I-3 NUMBERED clauses (gate #43, slot 65) MUST be extended in the next backlog cycle to load the canonical table from `00-gate-slot-binding.md` as a fourth ledger source (alongside slot-file headers, workflow YAML, and the retired-set constant). Until that extension ships, the four-way parity is enforced by `64-meta-verify-lockstep.md` clause-5 (gate #42) banner-triple recount, augmented by reviewer attestation that the canonical-table row count was updated in the same PR. The carriers-exclusion glob change (B-22) is enforced by reviewer attestation today + same-PR sweep that any new `_carriers/` file is reflected in `00-carriers-namespace-migration.md` §1 roster table within ≤ 1 session drift.
