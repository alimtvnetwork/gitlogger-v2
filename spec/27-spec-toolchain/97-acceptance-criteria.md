...
**Version:** 2.21.0
**Updated:** 2026-05-11 (Sess-69 B-19 — AC-T-39 added: per-slot self-sufficiency contract requiring active-gate slot files to inline a `## Red-green test pairs (AC-T-39)` section between `## Bindings` and `## Scorecard impact`. 5-clause structural contract (heading literal + RED→GREEN order + executable invocation + fixture-citation + no-restate). Phased rollout: Sess-69 B-19 lands the convention into the 5 highest-leverage active-gate slots (#36/#39/#40/#41/#42 = slots 58/61/62/63/64); remaining 21 active-gate slots tracked under B-19b. Reverse-coverage invariant: any newly-active-gate slot MUST add the section in the same commit or opt out under `## Out of scope`. Mechanically enforced by gate #42 clause-5 today + future clause-6 `active-gate-test-pair-presence` extension. AC count 38 → 39; module-level total 39 + delegated ≥100 = ≥139. Lifts §27 R-band C3 (Testability) 12 → ~13.5 (partial — full +4 lift gated on B-19b completion). Closes B-19 (phase-1) from Sess-69 remaining-tasks list.)
