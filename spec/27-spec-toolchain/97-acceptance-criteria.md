...
- **Worked example (verifier sketch):**
  ...
  - `for f in $non_gate_slots; do grep -qE '^\*\*Test pair:\*\*\s+N/A\s+—\s+(advisory|generator|scaffold|audit-aid|convention)\b' "$f" || echo "MISSING: $f"; done` MUST print **zero lines** at AC-T-41 ceiling.
  - Phased rollout: today (Sess-73 B-28) lands the AC contract + axis-class enum + verifier sketch; per-slot stub insertion is reviewer-attestation only, batched in a future spec-only `next` turn that walks the 53 files. Until per-slot ceiling reached, the verifier above prints up to 53 lines (the gap surface), not zero.
