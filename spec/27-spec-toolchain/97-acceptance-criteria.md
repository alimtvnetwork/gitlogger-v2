...
- **Phased rollout (this AC's own scope):** Sess-69 B-19 inlines the convention into the **5 highest-leverage active-gate slots** (#36 = `58-check-no-sql-ddl-in-ui-folder.md`, #39 = `61-check-no-out-of-scope-spec-folder-link.md`, #40 = `62-check-ci-cli-self-test-harness.md`, #41 = `63-check-diagram-parity.md`, #42 = `64-meta-verify-lockstep.md`). The remaining **21 active-gate slots** (#20..#35, #37, #38, #43..#46 minus retired) are tracked under **B-19b** (next backlog batch). Until B-19b lands, AC-T-39 is satisfied by reviewer attestation that newly-modified active-gate slots ship with a test pair; the full 26-slot ceiling enforcement is gated on B-19b completion.
...
- **Worked example (verifier sketch):**
  ...
  - `for f in $active_gates; do grep -q "^## Red-green test pairs (AC-T-39)" "$f" || echo "MISSING: $f"; done` — under the Sess-69 B-19 phased rollout, this loop is allowed to print the 21 not-yet-converted slots; under B-19b ceiling enforcement it MUST print zero lines.
