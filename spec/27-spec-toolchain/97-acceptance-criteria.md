...
**Version:** 2.23.0
**Updated:** 2026-05-11 (Sess-69 B-22 — AC-T-37 amended with **Carriers exclusion clause**: files under `spec/27-spec-toolchain/_carriers/` MUST NOT be counted in `disk_count` or `table_count` for the four-way parity invariant; the recount glob is anchored numeric-prefix `spec/27-spec-toolchain/[0-9]*.md`. Forward-compatible: until the deferred-implement turn lands the file moves, the glob change is a no-op (every active slot today carries a numeric prefix). Authored alongside `00-carriers-namespace-migration.md` v1.0.0 (B-22 spec doc with the 6-file Phase-1 carrier roster + deferred-implement turn checklist) — amendment text quoted verbatim from that file's §3.1. AC count carried at 39 (AC-T-37 body amendment only). Lifts §27 R-band C6 (Friction) **14 → 15** (partial +1; full +2 to band-anchor 16 gated on the deferred-implement turn) and C4 (Consistency) **14 → 15** (partial +1; cumulative trajectory: 14 → 16 once B-20-impl + B-16-impl + B-22-impl all land).)
**Version-prev:** 2.22.0
