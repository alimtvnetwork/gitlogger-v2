...
**Version:** 4.24.0  
**Updated:** 2026-05-10 (Phase-5 T-32 — added slot 61 `check-no-out-of-scope-spec-folder-link.py`, gate #39. **Fourth and FINAL boundary-quartet gate** — completes on-disk mechanisation of `mem://constraints/spec-scope`. Closes §27 backlog `no-out-of-scope-spec-folder-link-in-locked-7` minted Phase-5 T-09 (paper-only for 21 cycles) — **FINAL §27 backlog ticket** (§27 backlog reaches zero). 6-clause locked-7 perimeter scan over all 7 in-scope folders: no `spec/(00-21|29)-…/` path tokens (adjacency-marker exemption for `out-of-scope`/`archived`/`superseded` mentions) / no `spec/_archive/` path tokens (same exemption shape) / no Markdown links to out-of-scope paths (no exemption — links assert active referenceability) / no fenced embeds with out-of-scope source attribution / §27 §00 in-scope enumeration `Locked-7 in-scope folders: §22, §23, §24, §25, §26, §27, §28` + Lesson #15 gate self-citation / slot doc cites `mem://constraints/spec-scope`. Built-in 6-fixture in-memory `--self-test`. §27 advances Cursor 118 → 120, Raw-LLM 116 → 119 (§27 reaches 120/120 ceiling for Lovable + Cursor personas). All 7 folders gain C4 +1 (in-scope status load-proven from inside). Boundary quartet 4/4 closed; §27 backlog drained. Prior: T-31 — slot 60 `check-no-toolchain-enum-in-issues-folder.py`, gate #38.)
