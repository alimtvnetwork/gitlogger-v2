...
**Version:** 4.17.0  
**Updated:** 2026-05-10 (Phase-5 T-25 — added slot 54 `check-seed-id-explicit-locked-form.py`, gate #32. First §23-seed-surface gate; closes §27 backlog `seed-id-explicit-locked-form-check` minted T-10 — oldest §23-side backlog ticket (paper-only for 13 cycles). 6-clause locked-ID seed shape scan over §23 §00 seed-data section (line 199): PRIMARY-lane explicit-PK literals + locked tuples / REFERENCE-lane parity-pinned form with explicit `ON CONFLICT (col)` targets / parity matrix bidirectional row coverage / forbidden-shape block literal pinning / cite chain `AC-ADB-13`+`AC-ADB-11`+`T-10 remediation`+`MATERIALISE`+`DO NOT MATERIALISE` / no fenced-SQL restate of locked-ID INSERT in §22/§24/§25 children. Built-in 6-fixture in-memory `--self-test`. Promotes AC-ADB-13 from conditional 20 to un-conditional 20. §23 advances toward 19-20 self-enforcing band (Lovable 116 → 119, Cursor 112 → 116, Raw-LLM 106 → 110). Prior: T-24 — slot 53 `check-appshell-route-matrix.py`, gate #31.)  
