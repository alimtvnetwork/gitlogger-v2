...
**Version:** 4.41.0  
**Updated:** 2026-05-10 (Phase-5 G-6a — slot 56 `check-rest-pascalcase-parity.py` shipped on disk; gate #34 promoted from phantom citation to load-proven via built-in `--self-test` (6/6 fixtures pass); workflow step "§23 REST PascalCase parity gate" wired in `.github/workflows/spec-health.yml` directly after gate #33. Self-test runs hard; full disk run wired warn-only because §23 §00 R-2 ↔ PRIMARY-lane DDL bijection has known drift (`Name`/`AppName`, `LinkId`/`AppLinkId`, `DiscriminatorId`/`AppLinkTypeId`, `TargetKey`, `RepoUrlCanonical`) — separate G-6a-followup ticket reconciles §23 §00 wire shapes against DDL column-name set, after which the disk run flips to hard-fail. Re-running gate #43: phantom-script citations 26 → 25; resolution rate 64 % → 66 %. `meta-verify-lockstep` still 0 violations. AC-ADB-REST-01 promoted conditional 20 → load-proven 20. Scorecard: §23 R-band Cursor 117 → 118 (C3 Testability +1); §27 R-band C6 Friction +1 (oldest §23-side backlog ticket `rest-pascalcase-parity-check`, paper-only 21 cycles, retired). G-5 prior wave: legacy-name backfill cleared 8 phantom citations 34 → 26.)
