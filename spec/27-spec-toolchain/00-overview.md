...
**Version:** 4.59.0  
**Updated:** 2026-05-10 (Phase-5 G-6o / T-30 — slot 59 `check-no-ci-yaml-in-issues-folder.py` SHIPPED on disk; gate #37 self-test (6/6 fixtures: complete-clean + ci-fence + yml-path + bare-ci + ac-exemption-broken + boundary-stripped) + disk (5 clauses across 12 §25 .md files: 0 CI workflow YAML fences (existing §25 §97 `FindingStatus` enum YAML fence correctly classified as plain-config-shape — body lacks `runs-on:`/`uses: actions/`/top-level `jobs:`/`on:`); 0 §25-owned `.yml`/`.yaml` ownership claims; 0 bare CI literals in unfenced prose; AC-AI-13 R5 anchor exercised — `Cross-cite (§27 boundary gate)` line cites `\`check-error-envelope-uniformity\`` + `\`check-no-sql-ddl-in-ui-folder\`` + `\`check-no-ci-yaml-in-issues-folder\`` in backtick spans; §25 §00 boundary declaration + gate self-citation present) both green on FIRST run after two §25 surgical edits (line 44 `### Placement Rule` separator → No-CI-YAML boundary normative block carrying clause-5 literals; AC-AI-13 line 128 `Cross-cite (§27 boundary gate)` bullet for clause-4 R5 anchor). Workflow step `§25 no-CI-yaml boundary gate (#37 / T-30)` wired in `.github/workflows/spec-health.yml` immediately after gate #32 (gate #36 already wired at line 427 from prior session — duplicate citation removed). Five clauses: clause-1 no CI workflow YAML fences (yaml/yml fence whose body matches CI shape detector — plain config-shape YAML like `FindingStatus` enum permitted); clause-2 no §25-owned `.yml`/`.yaml` paths in `workflows/`/`ci/`/`github/` subdirs nor ownership-verb claims (`owned by §25`/`defined in §25`/`ships from §25`); clause-3 no bare CI literals (`runs-on: ubuntu-latest`, `uses: actions/checkout@`) in unfenced prose; clause-4 R5 anchor — ≥1 ### AC-AI-* section MUST cite a §27 gate name in a backtick span; clause-5 §25 §00 boundary literal pinning + gate self-citation per Lesson #15. Sibling chain: gate #36 (§24 no-DDL boundary) + **gate #37** (§25 no-CI-yaml boundary) form the **boundary-quartet** position 2/4 of the 4-gate scope-lock mechanization (slots 58/59/60/61). Closes §27 backlog `no-ci-yaml-in-issues-folder-check` (minted Phase-5 T-09). Phantom-script ledger **12 → 11** (-1 net; cumulative -23 across G-5 + G-6a..G-6o). Banner-triple gate count normalised to truthful **twenty-three** (recount basis below) (recount basis: `grep -lE '^\*\*Status:\*\*\s+Active\s+gate\s+#' spec/27-spec-toolchain/*.md | wc -l` = 23). `meta-verify-lockstep.py` still 0 violations; gates #23..#37 disk runs all green. Scorecard: §25 R-band C4 (Consistency) +1 (no-CI-yaml boundary now self-enforcing — closes T-09 backlog sibling); §27 R-band C6 +1 (one more backlog ticket retired).  
**Prior banner — Version:** 4.58.0; **Updated:** 2026-05-10 (Phase-5 G-6n / T-29 — slot 58 `check-no-sql-ddl-in-ui-folder.py` shipped; gate #36; phantom 13 → 12; §24-as-render-only triangle complete.)
