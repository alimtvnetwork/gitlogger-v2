...
**Version:** 4.18.0  
**Updated:** 2026-05-10 (Phase-5 T-26 — added slot 55 `check-dialect-precedence-banner-present.py`, gate #33. First §22+§23 cross-folder dialect-precedence gate; closes §27 backlog `dialect-precedence-banner-present` minted T-08 — second-oldest §23-side backlog ticket (paper-only for 14 cycles). 6-clause dialect-precedence banner scan: precedence heading positionally BEFORE first SQL fence / lane table closed-set 2 rows with all literals / cross-cuts block 6 cross-AC literals / AI-walker contract literals (`partial-context violation`+`re-anchor`) / every §23 child SQL fence preceded by lane marker within 30 lines / §22 single-dialect declaration `Database engine`→`SQLite`. Built-in 6-fixture in-memory `--self-test`. Promotes AC-ADB-11 from conditional 20 to un-conditional 20. §23 advances toward 120/120 ceiling (Lovable 119 → 120, Cursor 116 → 117, Raw-LLM 110 → 112). Prior: T-25 — slot 54 `check-seed-id-explicit-locked-form.py`, gate #32.)  
