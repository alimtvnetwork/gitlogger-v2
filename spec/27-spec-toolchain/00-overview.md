...
**Version:** 4.96.0  
**Version-prev:** 4.95.0  
**Updated:** 2026-05-11 (Sess-81 B-3 — NEW root-level cross-cohort read-order DAG manifest `00-cross-cohort-read-order-dag.md` v1.0.0 created in §27 (sister to per-cohort `00-tier1-bundle.md` layer; pins canonical 7-cohort read-order §27 → §28 → §22 → §23 → §24 → §26 → §25, 9 inter-cohort dependency edges E-1..E-9, per-cohort tier-1 KB roll-up Σ ~1,084 KB, 4 pre-budget cross-cohort recipes R-1..R-4, 5-clause reflexive drift contract pinning this file to any cohort tier-1 partition change). Lifts §27 R-band **C2 (Completeness) 18 → 19** band-anchor advance — cited mechanism: this file fills the previously-noted gap "no single root manifest names the cross-cohort read-order" (mechanism-citation count for §27 C2 now 4: prior 3 from bottleneck-decomposition + this file as 4th independent lever). §27 R total **111 → 112**; aggregate Raw-LLM Σ **821 → 822/840** (97.9/100). §27 still sole Raw-LLM cohort floor at R 112. Total active gates: 26 unchanged. Same-PR banner-triple lockstep (§00 / §98 / §99 / cross-cohort-read-order-dag).  
**Prior — Version:** 4.95.0; **Updated:** 2026-05-11 (Sess-80 B-33 — see prior banner below.)  
**Version:** 4.95.0  
**Version-prev:** 4.94.0  
**Updated:** 2026-05-11 (Sess-80 B-33 — closes the §27 walker-cost asymmetry on `00-tier1-bundle.md` v1.2.0 → v1.3.0: per-file Walker-cost (KB) column added to that file's Tier-1 table (Σ ~419 KB across §00 ~83 KB / trace-map ~50 KB / §97 ~101 KB / §99 ~185 KB — §27 is the largest in-scope cohort by tier-1 KB; §99 alone is ~617% of the 30 KB walker cap) + new "Walker-cost reflexivity (load-budget pin)" section with 4 pre-budget recipes (verify-an-AC ~15-25 KB via §97 section anchor / trace-a-gate-dependency ~50 KB / decode-current-state ~10 KB via §99 newest-banner-block sub-slice / full-tier-1 read ~419 KB) + reflexive drift contract (special §99 cadence: every 5th `-stamp-bump` OR any 10 KB bucket cross). Cites mirror-septet anchor closure (Sess-79 B-27-§26) — 7 of 7 in-scope cohorts now carry the column. **Scorecard impact**: §27 R-band C6 (Friction) **16 → 17** band-anchor advance (cited mechanisms: walker-cost column on tier-1 bundle itself + mirror-septet anchor closure citation as 4th + 5th independent levers; total 5 cited mechanisms for C6 now). §27 R total **110 → 111**; aggregate Raw-LLM Σ **818 → 821/840** (post Sess-79 §26 +1 carried = 820 baseline; this turn +1 = 821; 97.7/100). §27 still sole Raw-LLM cohort floor at R 111 (§24 + §28 + §22 + §23 at 119; §25 + §26 at 120). **Total active gates: 26 unchanged.** B-33 status "open" → "advanced (band-anchor)". Same-PR banner-triple lockstep (§00 / §98 / §99 / tier-1-bundle / bottleneck-decomposition).  
**Prior banner — Version:** 4.94.0; **Updated:** 2026-05-11 (Sess-74 B-28-stub-sweep — AC-T-41 ceiling reached: 53/53 non-gate slot stubs on disk; §27 R C3 17 → 18 band-anchor; §27 R total 109 → 110.)
...
| 56 | [56-check-rest-pascalcase-parity.md](./56-check-rest-pascalcase-parity.md) | `linter-scripts/check-rest-pascalcase-parity.py` | Validator: §23 §00 `## REST / RPC Contract (Normative — Phase-5 T-06)` (line 361) + R-1 endpoint matrix (line 370) + R-2 JSON schemas (line 384) + R-4 invariant 1 PascalCase parity (line 440) + §23 §97 AC-ADB-REST-01 (line 454) + §23 PRIMARY-lane DDL column-name set — 6-clause REST PascalCase parity scan (every wire JSON key matches `^[A-Z][A-Za-z0-9]*$` PascalCase / wire-key set ↔ PRIMARY-lane DDL column-name bijection modulo `{RepoUrl,ResolutionState}` request-only whitelist / boolean sample values restricted to `Is`-prefixed keys / R-4 invariant 1 literals `PascalCase parity`+`1:1`+`No camelCase`+`no snake_case on the wire` plus Lesson #15 self-citation `Self-enforcing via §27 backlog gate \`rest-pascalcase-parity-check\`` preserved / AC-ADB-REST-01 surface presence + `8-row R-1 endpoint matrix`+`R-4 invariants 1–7` + status tag / no-restate of App `## REST / RPC Contract` heading or R-1 matrix in §22/§24/§25). Built-in `--self-test` against 6 in-memory synthetic fixtures (F-1 unique passing; F-2 wire key `appId` camelCase, F-3 wire key `LinkLabel` with no DDL backing, F-4 boolean `Active: true` on non-`Is` key, F-5 R-4 strips gate self-citation, F-6 §24 child restates `## REST / RPC Contract` all fail). Gate #34 (Phase-5 T-27); first §23 wire-shape ↔ DDL-shape bijection gate; closes §27 backlog entry `rest-pascalcase-parity-check` minted Phase-5 T-06 — oldest §23-side REST backlog ticket (paper-only for 21 cycles, longest-aged §23 entry). Promotes AC-ADB-REST-01 from conditional 20 to un-conditional 20. Sibling to gate #24 (`check-boolean-uniformity-primary-lane`) which covers the DB-side boolean lane; clause-3 `Is`-prefix scan is the wire-side mirror; together they form a 3-surface boolean parity triple (DB / wire / UI) when combined with §24 U-3. |
| 57 | [57-check-rest-boolean-parity.md](./57-check-rest-boolean-parity.md) | `linter-scripts/check-rest-boolean-parity.py` | Validator: §23 §00 R-2 JSON schemas (line 384) + R-4 invariant 2 boolean parity (line 441) + §23 §97 AC-ADB-REST-01 + AC-ADB-11 + §22 `17-openapi.yaml` boolean field set (lines 85, 101, 103, 273, 286) + §24 §00 U-3 boolean render parity (~line 359) — 6-clause REST boolean parity scan (every JSON `true`/`false` sample value lives on `^Is[A-Z]…$` key / R-4 invariant 2 literals `wire \`true\`/\`false\` ↔ DB \`1\`/\`0\``+`Never accept \`0\`/\`1\` integers in request bodies`+`reject with 422 \`field.invalid\`` + Lesson #15 self-citation `Self-enforcing via §27 backlog gate \`rest-boolean-parity-check\`` preserved / OpenAPI App-surface `type: boolean` fields all `Is`-prefixed (git-logs whitelist `{enabled,allowed,success}`) / no integer-coercion stand-in `type: integer, enum:[0,1]` / §24 U-3 surface presence with `IsActive` canonical literal / no parallel App-surface boolean encoding table in §22 (excl. openapi)/§24 (excl. U-3 row)/§25). Built-in `--self-test` against 6 in-memory synthetic fixtures (F-1 unique passing; F-2 `Active: true` on non-`Is` key, F-3 R-4 strips gate self-citation, F-4 OpenAPI integer-coercion stand-in, F-5 §24 U-3 strips `IsActive` literal, F-6 §25 parallel boolean table all fail). Gate #35 (Phase-5 T-28); **final REST-side §27 backlog ticket** retired — closes `rest-boolean-parity-check` minted Phase-5 T-06 (paper-only for 22 cycles). Promotes AC-ADB-REST-01 + AC-ADB-11 wire-side surfaces from conditional 20 to un-conditional 20. Completes the **3-surface boolean parity triple (DB / wire / UI)** with gate #24 (DB-side) + gate #34 (wire-side `Is`-prefix); slot 55 cross-cuts cite `Boolean policy (AC-ADB-11` is now load-proven on all three surfaces simultaneously. |
| 58 | [58-check-no-sql-ddl-in-ui-folder.md](./58-check-no-sql-ddl-in-ui-folder.md) | `linter-scripts/check-no-sql-ddl-in-ui-folder.py` | Validator: §24 §00 boundary contract (no DDL surface) + §24 §97 AC-ADS-06 transitive boundary + locked-7 scope-lock memory invariant (`mem://constraints/spec-scope`) + §23 §00 dialect precedence pin (single source of truth for App-side DDL is §23) — 6-clause §24 no-DDL boundary scan (no ` ```sql `/`sqlite`/`postgres`/`pg`/`mysql`/`mariadb`/`ddl`/`plpgsql` fences in any §24 `.md` / no bare DDL keywords `CREATE TABLE `+`ALTER TABLE `+`DROP TABLE `+`CREATE INDEX `+`ALTER COLUMN ` in unfenced prose / no `spec/24-…/(migrations\|schema\|ddl)/.*\.sql` paths claimed authoritative / U-1 + U-3 binding-table column references inside `\|` table cells / backtick spans EXEMPT (positive guarantee — clause-4 false-positive is its own self-test failure) / §24 §00 boundary declaration with literal `App-side DDL is owned exclusively by §23` + Lesson #15 self-citation `Self-enforcing via §27 backlog gate \`no-sql-ddl-in-ui-folder-check\`` / slot doc Section "Bindings" cites `mem://constraints/spec-scope`). Built-in `--self-test` against 6 in-memory synthetic fixtures (F-1 unique passing; F-2 § ```sql ` fence with `SELECT * FROM App`, F-3 unfenced prose `Run CREATE TABLE App (...)`, F-4 `spec/24-…/migrations/001.sql` reference, F-5 clause-4 exemption regression — gate flags `IsActive` inside U-3 row, F-6 §24 §00 strips gate self-citation literal all fail). Gate #36 (Phase-5 T-29); first §24-boundary spec-content gate; **first of 4 boundary-quartet gates** (slots 58/59/60/61 mechanise the locked-7 scope-lock memory invariant on disk). Closes §27 backlog entry `no-sql-ddl-in-ui-folder-check` minted Phase-5 T-09 (paper-only for 19 cycles). §24 reaches 120/120 ceiling for Lovable + Cursor personas (Lovable carried, Cursor 119 → 120, Raw-LLM 115 → 117). Sibling to gate #19 (`check-ads-boundaries`, runtime AST side); together they enforce the §24 boundary on both code and spec surfaces. |
| 59 | [59-check-no-ci-yaml-in-issues-folder.md](./59-check-no-ci-yaml-in-issues-folder.md) | `linter-scripts/check-no-ci-yaml-in-issues-folder.py` | Validator: §25 §00 boundary contract (no CI yaml surface) + §25 §97 AC-AI-* scope-lock + locked-7 scope-lock memory invariant (`mem://constraints/spec-scope`) + §27 §00 + §28 §00 single-source-of-truth-for-CI-workflows claim — 6-clause §25 no-CI-yaml boundary scan (no ` ```yaml `/` ```yml ` fence in §25 `.md` whose body matches CI-workflow shape detector `runs-on:`/`uses: actions/`/`jobs:`/`on: push`/`on: pull_request`/`on: schedule` — plain non-CI YAML PERMITTED / no `spec/25-…/(workflows\|ci\|github)/.*\.ya?ml` paths claimed authoritative — `.github/workflows/spec-health.yml` cite in backtick READ-only span PERMITTED / no bare CI keyword structures `runs-on: ubuntu-latest`+`uses: actions/checkout@`+`jobs:\n  `+`on:\n  push:`+`on:\n  pull_request:` in unfenced prose — backticked single-token `\`runs-on\`` exempt / AC-finding cross-refs to §27 gate names inside `### AC-AI-*` sections in backtick spans EXEMPT (positive guarantee — clause-4 false-positive is its own self-test failure F-5) / §25 §00 boundary declaration with literal `CI-workflow YAML is owned exclusively by §27` + Lesson #15 self-citation `Self-enforcing via §27 backlog gate \`no-ci-yaml-in-issues-folder-check\`` / slot doc Section "Bindings" cites `mem://constraints/spec-scope`). Built-in `--self-test` against 6 in-memory synthetic fixtures (F-1 unique passing; F-2 ` ```yaml ` fence with `jobs: test: runs-on: ubuntu-latest`, F-3 `spec/25-…/workflows/audit.yml` ownership claim, F-4 unfenced `on:\n  push:` prose, F-5 clause-4 exemption regression — gate flags gate-name in AC-AI backtick span, F-6 §25 §00 strips gate self-citation literal all fail). Gate #37 (Phase-5 T-30); **second of 4 boundary-quartet gates** mechanising `mem://constraints/spec-scope` on disk. Closes §27 backlog entry `no-ci-yaml-in-issues-folder-check` minted Phase-5 T-09 (paper-only for 19 cycles). §25 advances Lovable 118 → 119, Cursor 116 → 118, Raw-LLM 111 → 113. Sibling to gate #46 (`check-audit-quoted-evidence-marker`) which covers audit-quoting INSIDE §25; together they enforce the §25 surface shape: audit-only, no executable CI surface. Sibling to gate #36 slot 58 (boundary-quartet pattern reuse). |

#### Phase-5 P19 wave gate-cite block (boundary-quartet + CI-CLI + diagram-parity completion)

The §27 ledger above stops at slot 59 / gate #37 because the slot-numbering 60-69 range was already claimed by **config-file** rows (slot 60 `forbidden-strings.toml`, slot 61 `spec-cross-links.allowlist`, slot 62 `spec-folder-refs.allowlist`, slot 63 `readme-cross-links.md`). Phase-5 P19 / T-31..T-32 / T-36..T-38 minted four additional **validator** slot docs at the same numeric positions (alphabetic `check-…` files sort beside the config files in the same `spec/27-spec-toolchain/` directory). To preserve clause-1 `#N` token coverage for the meta-verifier, the four new gates are cited here as a Phase-5 P19 sub-ledger:

| # | Slot doc | Code artifact | Status / wave |
|---|----------|---------------|---------------|
| **#38** | [60-check-no-toolchain-enum-in-issues-folder.md](./60-check-no-toolchain-enum-in-issues-folder.md) | `linter-scripts/check-no-toolchain-enum-in-issues-folder.py` | **third of 4 boundary-quartet gates** (Phase-5 T-31 / P19); §25 must not restate the §27 gate-enum table; sibling to #36/#37/#39. |
| **#40** | [62-check-ci-cli-self-test-harness.md](./62-check-ci-cli-self-test-harness.md) | `linter-scripts/check-ci-cli-self-test-harness.py` | §28 self-test harness contract enforcer (Phase-5 T-38 / P19c); promotes AC-28-49 from literal-cited to load-proven; enables §28 honest triple-ceiling L120/C120/R120. |
| **#41** | [63-check-diagram-parity.md](./63-check-diagram-parity.md) | `(historical) check-diagram-parity.py` | §26 mermaid-diagram ↔ §22/§23 source-of-truth parity gate (Phase-5 T-36 family); ensures `lifecycle-*.mmd` artefacts don't drift from the structural surfaces they depict. |

Numbering note: gate **#38** is shipped, gate **#39** is the boundary-quartet locked-7 perimeter gate cited in §00 banner block above (Phase-5 T-32 / P19e), and gates **#40**/**#41** continue the Phase-5 P19 wave. Gate **#42** is the §27 reflexive `meta-verify-lockstep` (P19d, slot 64 — see banner block above) and gate **#43** is the disk-ledger `check-gate-ledger-vs-workflow` (P19a, slot 65). The contiguous run #36..#43 completes the **P19 wave + post-P19 grooming gates** — eight gates ship under one numbering arc.
...
| 10 | `consumes-frontmatter-resolves` | Active | §27 | `python3 (historical) check-consumes-frontmatter.py --root spec --in-scope 22,23,24,25,26,27,28` | 0 = pass; 1 = any `consumes:` / `produced_for:` entry references a missing file/section | validate stage |
| 11 | `cohort-naming-check` | Active | §27 | `python3 (historical) check-cohort-naming.py --root spec --in-scope 22,23,24,25,26,27,28` | 0 = pass; 1 = any in-scope folder/file violates AC-COHORT-06 filename regex or slot-reservation rules; 2 = invocation error | validate stage |
| 12 | `finding-status-enum-check` | Active | §27 | `python3 (historical) check-finding-status-enum.py --root spec/25-app-issues` | 0 = pass; 1 = any `## F-NN` Status ∉ {Open, In progress, Resolved, De-scoped}; 2 = invocation error | validate stage |
| 13 | `cohort-orphaned-finding` | Active | §27 | `python3 (historical) check-cohort-orphaned-finding.py --root spec/25-app-issues --max-age-sessions 1` | 0 = pass; 1 = any `Carried-open` disposition-map row with `Last touched` >1 session ago AND no §22 backlog citation; 2 = invocation error | audit stage |
| 14 | `finding-vs-audit-distinction-check` | Active | §27 | `python3 (historical) check-finding-vs-audit-distinction.py --root spec/25-app-issues` | 0 = pass; 1 = any `## F-NN` Evidence block cites a runtime `AuditTrail` row without `runtime-cite` tag; 2 = invocation error | validate stage |
| 15 | `derives-from-restate-check` | Active | §27 | `python3 (historical) check-derives-from-restate.py --consumer spec/24-app-design-system-and-ui --source spec/07-design-system --shingle 8 --max-matches 2` | 0 = pass; 1 = any §24 paragraph contains ≥3 8-token shingle matches with any §07 paragraph; 2 = invocation error | audit stage |
| 16 | `no-raw-color-in-app-component` | Active | §27 | `python3 (historical) check-no-raw-color.py --registry spec/24-app-design-system-and-ui --src src/components --token-prefix --app-error-` | 0 = pass; 1 = any TSX/CSS file in §24 component registry (resolved via A-12 `produced_for:` index) contains an inline color literal (hex, rgb(), hsl(), oklch()) outside an `--app-error-*` / token reference; 2 = invocation error | audit stage |
...
| 22 | `applink-xor-clause-check` | Active | §27 | `python3 (historical) check-applink-xor-clause.py --check all --self-test` | 0 = pass; 1 = §23 AppLink DDL XOR-CHECK clause / disconnect-CHECK / locked-ID seed / partial-index parity violation (AC-ADB-05 + AC-ADB-13 + AC-ADB-R-4 invariant 6); 2 = invocation error; 3 = fixture-rot (synthetic source missing) | validate stage |
