...
**Version:** 4.50.0  
**Updated:** 2026-05-10 (Phase-5 G-6h — slot 46 `check-audit-quoted-evidence-marker.py` shipped; gate #27 self-test + disk both load-proven (6/6 fixtures + 5 clauses); workflow step `§24+§25 audit-quoted-evidence marker gate` wired in `.github/workflows/spec-health.yml` directly after the idempotency-observability gate. Five clauses: caf05-marker (§24 §97 AC-CAF-05 carries `auditor-quoted evidence`/`spec/_archive/21-git-logs-v1/`/`AC-AI-10/11`/`cross-cutting status`), ai-10-11-surface (§25 §97 AC-AI-10 + AC-AI-11 present with required literals), ai-14-rule (§25 §97 AC-AI-14 rule 4 carries `Evidence snippets MUST be backticked or fenced` + `paraphrased evidence is FORBIDDEN` + `AC-AI-10`), finding-body-quoting (every foreign-AC ID `AC-(ALW|ERR|JWT|CG|SAG|TOK)-\d+` in §25 finding bodies MUST be backticked/fenced/blockquote), no-restate-23-24 (any foreign-AC mention in §23/§24 prose MUST be backticked AND have an evidence-marker word in ±2-line window). R5 vacuous-pass: zero AC-CAF-05 / AC-AI-10/11/14 anchors → exit 3 fixture-rot. **AC-CAF-05 (T-12)** promoted conditional 20 → load-proven 20 — **fifth and final cross-cutting CAF gate** after gates #23/#24/#25/#26. **Entire CAF-01..05 family un-conditionally load-proven** for the first time since T-12. Six surgical backtick insertions calibrated the disk surface: §25 phase-2 status-line `AC-SAG-04`; §23 §97 lines 164/176/204/215 `AC-CG-21`/`AC-CG-22` mentions (citing out-of-scope spec/02 — auditor-evidence by design per scope-lock memory). Phantom-script ledger **19 → 18** (-1 net; cumulative -16 across G-5 + G-6a..G-6h). `meta-verify-lockstep.py` still 0 violations; gates #23/#24/#25/#26/#33/#34/#35 disk runs still 0 violations. Scorecard: §24 R-band C2 +1 (91 → 92); §25 R-band C3 +1 (117 → 118); §23 R-band durability lift (already at ceiling); §27 R-band C6 +1 (closes 5/5 T-12 cluster — `audit-quoted-evidence-marker-check`).
