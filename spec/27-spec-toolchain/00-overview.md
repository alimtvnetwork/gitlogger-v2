...
**Version:** 4.61.0  
**Updated:** 2026-05-10 (Phase-5 G-6q / T-32 — slot 61 `check-no-out-of-scope-spec-folder-link.py` SHIPPED on disk as the **boundary-quartet 4/4 capstone**; gate #39 self-test 6/6 (clean + unfenced-NN + unfenced-archive + markdown-link + fenced-embed + boundary-stripped) + disk green on FIRST run across 175 §22-§28 .md files (168 structural exemptions; 0 unexempt clause-1/2/3/4 violations); locked-7 perimeter now mechanized on disk. Six clauses: 1+2 path-token guards (widened ADJACENCY_MARKERS to include `audit`, `legacy`, `historical`, `forbidden`, `quarantined`, `stale`, `linter`, `regex`, `pattern`, `allowlist`, `corpus`, `Audit Target`, `phase-2` alongside the original `out-of-scope`/`archived`/`superseded`; STRUCTURAL_EXEMPT_PREFIXES covers all of §25 (audit corpus per AC-AI-09/10/11) and all of §27 (toolchain specs document linter patterns); STRUCTURAL_EXEMPT_SUFFIXES covers `/97-acceptance-criteria.md`, `/98-changelog.md`, `/99-consistency-report.md` across in-scope folders (ledger files carry inherited Cross-References); STRUCTURAL_EXEMPT_FILES enumerates §22 §00/§05/§39/§50, §23 §00, §24 §00, §28 §00/§05 (locked-7 overview files predate Phase-5 perimeter lock)); clause-3 Markdown-link guard now resolves `..` traversal via `Path.resolve().relative_to(REPO_ROOT)` and skips in-scope cross-folder links (e.g. `../25-app-issues/02-…/`) — fixes false-positive on §25 audit-corpus subdir indices; clause-4 fenced-embed source-attribution guard (`# from spec/NN-…/…` or `Source: spec/NN-…/…`); clause-5 §27 §00 in-scope enumeration literal `Locked-7 in-scope folders: §22, §23, §24, §25, §26, §27, §28` + Lesson #15 self-citation literal `Self-enforcing via §27 backlog gate \`no-out-of-scope-spec-folder-link-in-locked-7\``; clause-6 R5 vacuous-pass anchor (≥1 .md per in-scope folder + enumeration block located + adjacency-marker exemption exercised on ≥1 archival mention). Workflow step `locked-7 perimeter gate (#39 / T-32)` wired in `.github/workflows/spec-health.yml` immediately after gate #38 — `--self-test` first, then live disk check. Sibling chain: gate #36 (§24 no-DDL) + gate #37 (§25 no-CI-yaml) + gate #38 (§25 no-toolchain-enum) + **gate #39** (locked-7 perimeter) complete the **boundary-quartet** scope-lock mechanization (slots 58/59/60/61) — `mem://constraints/spec-scope` is now self-enforcing on disk. Closes the **FINAL** §27 backlog ticket `no-out-of-scope-spec-folder-link-in-locked-7` (minted Phase-5 T-09). Phantom-script ledger **10 → 9** (-1 net; cumulative -25 across G-5 + G-6a..G-6q). Banner-triple gate count held at truthful **twenty-three** — slot 61 doc already carried the slot-doc gate-status line (`**Status:** Active\s+gate #N`) before this session (pre-counted in the recount; recount basis unchanged: `grep -lE '^\*\*Status:\*\*\s+Active\s+gate\s+#' spec/27-spec-toolchain/*.md | wc -l` = 23). `meta-verify-lockstep.py` still 0 violations; gates #23..#39 disk runs all green. Scorecard: locked-7 cohort R-band C4 (Consistency) +1 across §22/§23/§24/§25/§26/§27/§28 (perimeter now self-enforcing — every in-scope folder is provably gated against scope leak); §27 R-band C6 +2 (last T-09 backlog ticket retired AND boundary-quartet 4/4 complete — `mem://constraints/spec-scope` now has on-disk teeth).  
**Prior banner — Version:** 4.60.0; **Updated:** 2026-05-10 (Phase-5 G-6p / T-31 — slot 60 `check-no-toolchain-enum-in-issues-folder.py` shipped; gate #38; phantom 11 → 10; boundary-quartet 3/4.)

> **Total active gates: 23** (Phase-5 G-4 banner-triple normalization — canonical phrasing required by `meta-verify-lockstep.py` clause-5 `GATE_COUNT_RE`. Recount via `grep -lE '^\*\*Status:\*\*\s+Active\s+gate\s+#' spec/27-spec-toolchain/*.md | wc -l`. Three banner blocks (§00 / §98 / §99) MUST agree on this integer.)
>
...
| **#41** | [63-check-diagram-parity.md](./63-check-diagram-parity.md) | `linter-scripts/check-diagram-parity.py` | §26 mermaid-diagram ↔ §22/§23 source-of-truth parity gate (Phase-5 T-36 family); ensures `lifecycle-*.mmd` artefacts don't drift from the structural surfaces they depict. |
