...
**Version:** 4.52.0  
**Updated:** 2026-05-10 (Phase-5 G-6j / T-22 — slot 48 `check-ac-prefix-contract.py` SHIPPED on disk; gate #29 self-test (6/6 fixtures) + disk (7 §97 files, 255 ACs, 10 prefix-roots, 0 violations across 5 clauses) both load-proven on first run; workflow step `§97 AC-prefix contract gate (#29 / T-22)` wired in `.github/workflows/spec-health.yml` immediately after the §97 structural-hygiene gate. Five clauses: clause-1 owner-folder (every `### AC-…` matches folder's owned-root set), clause-2 no-cross-folder-collision (every non-bare AC-ID globally unique), clause-3 bare-numeric-partition (no NEW bare-`AC-NN` collision across §22/§25/§26 outside the T-22 grandfathered baseline `{1..8, 22..26}` = 13 IDs — retro-renumbering 21 ACs would invalidate every external citation), clause-4 no-foreign-in-meta (§00/§98/§99 MUST NOT declare `### AC-…` foreign-prefix headers), clause-5 ownership-map-round-trip (every prefix-root present on disk MUST appear in the script's `KNOWN_ROOTS` table). Slot-doc revision (T-22 calibration): widened owned-root semantics to `AC-ADB-*`/`AC-ADS-*`/`AC-AI-*`/`AC-DG-*`/`AC-T-*` so existing sub-namespaces `AC-ADB-REST-NN`/`AC-ADB-SETTING-NN` (in §23 §00) + `AC-ADS-UI-NN` (in §24 §00) + `AC-AI-000` (in §25 §00) all pass clause-1 by-construction. Sibling to gate #28: gate #28 enforces within-file AC-ID uniqueness; gate #29 enforces cross-file AC-ID uniqueness — together they make the AC-namespace globally unique by machine-checked construction. Sibling to gate #27/AC-CAF-05 on the foreign-AC axis: gate #27 enforces foreign-AC quoting discipline in prose; gate #29 enforces foreign-AC declaration discipline in headers. R5 vacuous-pass: zero §97 files OR zero AC headers → exit 1. Phantom-script ledger **17 → 16** (-1 net; cumulative -18 across G-5 + G-6a..G-6j). `meta-verify-lockstep.py` still 0 violations; gates #23..#29 / #33/#34/#35 disk runs still 0 violations. Scorecard: §22/§23/§24/§25/§26/§27/§28 R-band C4 (Consistency) +1 each (cross-folder AC-ID collision detection + foreign-prefix declaration leak vector closed); §24 R-band C4 +1 additionally (CAF-NN namespace ownership mechanically pinned to §24 by ownership-map decree); §27 R-band C5 holds at 20 (gates #15+#28+#29 jointly self-enforce AC-namespace discipline) and C6 +1 (second-oldest backlog ticket T-14 closed; ownership map + exit-code enumeration in slot doc).  
**Prior banner — Version:** 4.51.0; **Updated:** 2026-05-10 (Phase-5 G-6i / T-21 — slot 47 `check-ac-section-orphan-header.py` shipped; gate #28 hard-fail self-test + disk; phantom 18 → 17.)
