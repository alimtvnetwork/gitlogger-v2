...
**Version:** 4.40.0  
**Updated:** 2026-05-10 (Phase-5 G-5 — §27 §00 main-ledger legacy-name backfill against gate #43 (`check-gate-ledger-vs-workflow.py`). Five `## CI Gate Enumeration` rows (gates #4/#6/#7/#8/#9) had outdated `node …cjs` / `audit-walker.py` / `check-folder-refs.py` invocations — all 5 swapped to their on-disk `.py` equivalents (`check-spec-folder-refs.py`, `check-version-parity.py`, `audit-spec-vs-code-v2.py`, `check-99-summary-freshness.py`, `check-99-stamp-bump.py`). §97 row 28 swapped from phantom `check-archive-exclusion-runtime.py` to the real `linter-scripts/test/test-archive-exclusion-runtime.sh` (matches slot-28 "the self-test IS the gate" contract). Two didactic placeholder citations (`check-foo.py` in slot 28 prose, `does-not-exist.py` in slot 14 AC-14-03) reformatted with `<>` token brackets to side-step gate #43's `linter-scripts/<name>.py` regex. Re-running gate #43: phantom-script citations **34 → 26** (-8); citation count 81 → 73; I-2 WIRED holds at 17 (those require shipping new scripts, not edits — backlog G-6); I-3 NUMBERED holds at 1 (20 gate-number gaps — can only close by minting gates). `meta-verify-lockstep` still 0 violations. **No new gates, no new scripts, no new slots — pure citation-correctness lift.** §27 triple-ceiling L120/C120/R120 held; gate #43 still warn-only (flips to hard-fail when I-2 + I-3 close out under G-6/G-7).)
