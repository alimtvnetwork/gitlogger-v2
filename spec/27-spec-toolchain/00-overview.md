...
**Version:** 4.42.0  
**Updated:** 2026-05-10 (Phase-5 G-6a-followup — §23 §00 R-2 wire shapes reconciled against PRIMARY-lane DDL column-name set: App body now {AppId,AppName,AppSlug,Description,ProfileId,AppStatusId,CreatedAt,UpdatedAt} — drops `Name`/`RepoUrlCanonical`/`IsActive`; AppLink body now {AppLinkId,AppId,AppLinkTypeId,TargetGitProfileId,TargetRepoId,IsActive,CreatedAt,DisconnectedAt} — replaces `LinkId`/`DiscriminatorId`/`TargetKey`. R-1 R-06/R-07 paths + return shapes updated to AppLinkId. R-3 envelope `Field` example updated to `AppLinkId`. AC-ADB-REST-01 heading gains `[active]` status tag (closes gate #34 clause-5). Re-running gate #34 disk run: **0 violations across all 6 clauses** (was 15). Workflow step "§23 REST PascalCase parity gate" flipped warn-only → **hard-fail** in `.github/workflows/spec-health.yml`. AC-ADB-REST-01 now fully load-proven (heading + body literals + status tag + 6-clause mechanical bijection). `meta-verify-lockstep.py` still 0 violations. Scorecard: §23 R-band Cursor 118 → 120 (C3 Testability +1, C4 Consistency +1 — wire ↔ DDL bijection mechanically pinned with hard-fail enforcer); Raw-LLM 112 → 114. Lovable §23 already at 120 carried.)
