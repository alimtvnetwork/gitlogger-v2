...
**Version:** 4.84.0  
**Updated:** 2026-05-11 (Sess-69 B-19 phase-1 — minted **AC-T-39** in §97 (per-slot self-sufficiency: active-gate slots MUST inline a `## Red-green test pairs (AC-T-39)` section between Bindings and Scorecard impact, structured RED→GREEN with executable invocations + fixture citations) and inlined the convention into the **5 highest-leverage active-gate slots** (#36/#39/#40/#41/#42 = `58-check-no-sql-ddl-in-ui-folder.md` / `61-check-no-out-of-scope-spec-folder-link.md` / `62-check-ci-cli-self-test-harness.md` / `63-check-diagram-parity.md` / `64-meta-verify-lockstep.md`). Each slot now ships 2 RED + 2 GREEN rows verified by `grep -cF '**RED:**'` and `grep -cF '**GREEN:**'` returning 2/2 across all 5 files. Phased rollout: remaining 21 active-gate slots tracked under B-19b. Reverse-coverage invariant + 5-clause structural contract pinned in §97. Mechanically enforced by gate #42 clause-5 today + future clause-6 `active-gate-test-pair-presence` extension. Lifts §27 R-band C3 (Testability) 12 → ~13.5 (partial; full +4 lift gated on B-19b ceiling). §97 v2.20.0 → v2.21.0 (AC count 38 → 39).  
**Prior banner — Version:** 4.83.0; **Updated:** 2026-05-11 (Sess-69 B-18 — Raw-LLM bottleneck decomposition `00-raw-llm-bottleneck-decomposition.md` shipped; per-criterion R-band hand-score + 5 named lift candidates with cited mechanisms; closed B-18 from Sess-68.)
