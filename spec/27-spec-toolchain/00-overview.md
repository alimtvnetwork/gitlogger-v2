...
**Version:** 4.44.0  
**Updated:** 2026-05-10 (Phase-5 G-6d — slot 43 `check-boolean-uniformity-primary-lane.py` shipped on disk; gate #24 load-proven via built-in `--self-test` (6/6 fixtures pass) AND disk run **0 violations across all 5 clauses + R5 vacuous-pass guard**; workflow step "§22+§23+§24 boolean uniformity (PRIMARY-lane) gate" wired hard-fail in `.github/workflows/spec-health.yml` directly after gate #35. Implements 5-clause cross-folder scan: clause-1 PRIMARY-lane DDL boolean discipline (every boolean-shaped column is `INTEGER` and `Is`-prefixed; bare `BOOLEAN`/`TINYINT` rejected; mirrors §22 `18-schema.sql` with `{HasError,PreviousHasError,Truncated}` legacy whitelist), clause-2 §22 OpenAPI wire-shape (predicate-prefix discipline `Is|Has|Was|Can|Should|Truncated|Deprecated` + no `type: integer + enum:[0,1]` integer-coercion stand-in), clause-3 §24 §00 U-3 render parity (literals `IsActive`/`Active`/`Inactive`/`MUST NOT render raw`/`MUST NOT invert`/`third`/`§23 R-4` + ≤14-line budget), clause-4 §23 §00 R-4 invariant 2 + R-1 `field.invalid` + WE-3 boolean-coercion attack heading present, clause-5 no-restate of parallel `Boolean encoding`/`Boolean policy`/`Boolean translation` headings in §22/§24/§25 children. **Closes the 3-surface boolean parity triple** (DB-side gate #24 + wire-side gate #34 PascalCase + wire-side gate #35 boolean) — AC-CAF-01 (T-12) flips from conditional 20 (paper-only / 3-turn decay) to un-conditional 20 (load-proven end-to-end DB ↔ REST ↔ UI). Phantom-script ledger 24 → 23. `meta-verify-lockstep.py` still 0 violations. Scorecard: §23 R-band Cursor 120 carried; §23 R-band Raw-LLM 116 → 118 (C3 Testability +1 + C5 Implementability +1 — DB-side boolean lane mechanically pinned for first time, supersedes gate #22 partial `IsActive`-only coverage). §22 + §24 R-band C4 Consistency +1 each (clause-5 no-restate forbids parallel boolean encoding tables in either folder; clause-3 hard-pins §24 U-3 render parity literal set). §27 R-band C6 Friction +1 (second-to-last cross-cutting CAF gate retires). AC-CAF-01 promoted to load-proven — second un-conditional CAF after T-16 gate #23 CAF-02.)
