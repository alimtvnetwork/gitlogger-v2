...
**Version:** 4.21.0  
**Updated:** 2026-05-10 (Phase-5 T-29 — added slot 58 `check-no-sql-ddl-in-ui-folder.py`, gate #36. **First of 4 boundary-quartet gates** mechanising the locked-7 scope-lock memory invariant (`mem://constraints/spec-scope`) on disk. Closes §27 backlog `no-sql-ddl-in-ui-folder-check` minted Phase-5 T-09 (paper-only for 19 cycles). 6-clause §24 no-DDL boundary scan: no 8-dialect SQL fences in §24 `.md` / no bare DDL keywords in unfenced prose / no `migrations|schema|ddl/*.sql` path claimed authoritative / U-1+U-3 binding-table column refs inside `|` cells + backtick spans EXEMPT (positive guarantee with self-test regression coverage) / §24 §00 boundary declaration `App-side DDL is owned exclusively by §23` + Lesson #15 gate self-citation / slot doc cites `mem://constraints/spec-scope`. Built-in 6-fixture in-memory `--self-test`. §24 reaches 120/120 for Lovable + Cursor personas (Lovable carried, Cursor 119 → 120, Raw-LLM 115 → 117). Prior: T-28 — slot 57 `check-rest-boolean-parity.py`, gate #35.)  
