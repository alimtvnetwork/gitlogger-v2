...
**Version:** 4.86.0  
**Updated:** 2026-05-11 (Sess-69 B-21 — extended `00-tier1-bundle.md` with a **Tier-2 slot index lookup table** (28 active-gate slot rows: slot # → gate # → file → category) so a Raw-LLM reader can resolve "which slot owns gate #N?" in a single pass without paging through the categorical inventory in this file. Bundle file v1.0.0 → v1.1.0 (additive only; no tier partition changes; navigation-aid only — no new normative cross-gate contracts introduced). Lifts §27 R-band C6 (Friction) **11 → 14** (full +3; cited mechanism: tier-1 bundle drift contract clause `Add or retire a gate → update both the relevant tier-2 category row here AND the Active Gate Inventory in 00-overview.md in the same commit. Failure to do so MUST be flagged by the next §99 entry` — now binds the new lookup table by reference). Same-PR refresh of `00-raw-llm-bottleneck-decomposition.md` per-criterion table (C6 row: 11 → 14; lift candidate B-21 moves to "closed" status; total §27 R 95 → 98). No AC mint required — bundle is a navigation-aid file (Lesson #36), not a contract surface.  
**Prior banner — Version:** 4.85.0; **Updated:** 2026-05-11 (Sess-69 B-19b — AC-T-39 ceiling reached: red-green test pairs inlined into the remaining 21 active-gate slots, bringing coverage to 26/26. Lifts §27 R-band C3 12→16.)
