...
**Version:** 4.20.0  
**Updated:** 2026-05-10 (Phase-5 T-28 — added slot 57 `check-rest-boolean-parity.py`, gate #35. **Final REST-side §27 backlog ticket** retired — closes `rest-boolean-parity-check` minted Phase-5 T-06 (paper-only for 22 cycles). 6-clause REST boolean parity scan: wire `true`/`false` sample values restricted to `^Is[A-Z]…$` keys / R-4 invariant 2 literals + Lesson #15 gate self-citation preserved / §22 OpenAPI App-surface `type: boolean` ↔ wire bijection (git-logs whitelist `{enabled,allowed,success}`) / no `type: integer, enum:[0,1]` integer-coercion attack surface / §24 U-3 surface pin with `IsActive` literal + AC-ADB-11/CAF-01 back-ref / no parallel boolean encoding table in §22/§24/§25. Built-in 6-fixture in-memory `--self-test`. Completes the **3-surface boolean parity triple (DB / wire / UI)** — gate #24 (DB) + gate #34 (wire naming) + gate #35 (wire value + OpenAPI + UI). §23 advances Cursor 118 → 119, Raw-LLM 114 → 115. Prior: T-27 — slot 56 `check-rest-pascalcase-parity.py`, gate #34.)  
