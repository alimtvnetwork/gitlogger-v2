...
**Version:** 4.48.0  
**Updated:** 2026-05-10 (Phase-5 G-6f — slot 44 `check-seedable-config-row-present.py` shipped; gate #25 self-test + disk both load-proven (6/6 fixtures + 6 clauses); workflow step `§24 seedable-config row gate` wired in `.github/workflows/spec-health.yml` directly after gate #23. Six clauses: s1-s2-coverage (every S-NN cites valid R-NN, accepting both §24 S-2 and §23 R-1 endpoints; orphan panels/endpoints rejected; S-01 layout-only exempt), seed-row-presence (R-10 byte-verbatim `partial body; never overwrites seed row` + `Setting` table backtick-named in S-3 prose), override-table-separation (S-3 invariant 2 carries all three literals `UserSettingOverride` + `ON CONFLICT(UserId,Key) DO UPDATE` + `MUST NOT mutate … the seed row`; collapsing any one is the canonical CAF-04 failure mode), r09-merged-view (R-09 note `merged seed + per-user override` + S-3 invariant 3 non-inverted `COALESCE(override.Value, seed.Value)`; inverted form rejected as silent-default-wins violation), forward-only-removal (S-3 invariant 4 cites `§23 Rule 12` + `seed row`/`overrides`/`same forward-only migration` literals all three), no-restate (§22/§23 §00 MUST NOT inline `Settings persistence contract` heading + COALESCE pair). R5 vacuous-pass: missing S-1/S-2/S-3 anchors → exit 3 fixture-rot. **AC-CAF-04 (T-12)** promoted conditional 20 → load-proven 20 — third cross-cutting CAF gate after gate #23 (CAF-02) and gate #24 (CAF-01). **AC-ADS-UI-03** machine-checked sibling clause satisfied without amending AC text. Phantom-script ledger **21 → 20** (-1 net; cumulative -14 across G-5 + G-6a..G-6f). `meta-verify-lockstep.py` still 0 violations; gates #23/#24/#33/#34/#35 disk runs still 0 violations. Scorecard: §24 R-band C3 Testability +1 (S-1/S-2/S-3 + AC-ADS-UI-03 surface load-proven; §24 R-band 89 → 90); §27 R-band C6 Friction +1 (paper-only-since-T-08 ticket `seedable-config-row-present-check` — 16 cycles, oldest §24 ticket — converts to load-proven; mirror of slot-43 G-6d zero-conversion-lag pattern but for an older ticket).
