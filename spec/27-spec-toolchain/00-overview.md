...
**Version:** 4.47.0  
**Updated:** 2026-05-10 (Phase-5 G-6e — slot 42 `check-error-envelope-uniformity.py` shipped; gate #23 self-test + disk both load-proven (5/5 clauses + 6/6 fixtures); workflow step `§22+§23+§24 ErrorEnvelope uniformity gate` wired in `.github/workflows/spec-health.yml` directly after gate #24. Five clauses: schema-pin (§22 exactly-one `ErrorEnvelope:` block; R5 vacuous-pass → exit 3), db-mirror (§23 §00 R-3 heading + WE-* non-2xx envelopes carry `Error.Code`+`Error.Message`+`TraceId`), ui-mirror (§24 AC-ADS-15 row + `<AppErrorState/>` matrix row cite `Error.Message`+`TraceId`), code-prefix (§22 GL-* / §24 ADS-* / §23 dotted-lowercase; foreign-prefix bleed rejected), no-restate (§23/§24 MUST NOT inline `ErrorEnvelope:` YAML block; Lesson #36). **AC-CAF-02 (T-12)** promoted conditional 20 → load-proven 20; this gate IS the cited self-enforcing mechanism. **AC-ADS-15** (§24 namespace extension) machine-checked sibling clause satisfied. **§22 AC-30** (`ErrorEnvelope` schema canonical cite) gains the 'implementation pending' resolver. Phantom-script ledger **22 → 21** (-1 net; cumulative -13 across G-5 + G-6a + G-6b + G-6c + G-6c-followup + G-6d + G-6e). `meta-verify-lockstep.py` still 0 violations; gates #24/#33/#34/#35 disk runs still 0 violations. Scorecard: §22 R-band C2 Completeness +1 (AC-30 schema-pin mechanically enforced); §23 R-band L120/C120/R120 carried (AC-CAF-02 lift accrues to §22 + §24); §24 R-band C3 Testability +1 (AC-ADS-15 + `<AppErrorState/>` contract load-proven); §27 R-band C6 Friction +1 (paper-only-since-T-12 ticket converts to load-proven on first turn — zero conversion lag).
