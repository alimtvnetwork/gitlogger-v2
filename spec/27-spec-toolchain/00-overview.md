...
**Version:** 4.23.0  
**Updated:** 2026-05-10 (Phase-5 T-31 — added slot 60 `check-no-toolchain-enum-in-issues-folder.py`, gate #38. **Third of 4 boundary-quartet gates** mechanising `mem://constraints/spec-scope` on disk. Closes §27 backlog `no-toolchain-enum-in-issues-folder-check` minted Phase-5 T-09 (paper-only for 20 cycles). 6-clause §25 no-toolchain-enum boundary scan: no parallel gate-enumeration table (header `Gate`+`Slot` with ≥3 rows) / no `linter-scripts/*.py` ≥4-item enumeration (list or fenced tree) / no slot-range table (`Range`+`Purpose`) / single-gate or single-slot citations in `### AC-AI-*` backtick spans EXEMPT (positive guarantee F-5 regression) / §25 §00 boundary declaration `Toolchain enumeration is owned exclusively by §27` + Lesson #15 gate self-citation / slot doc cites `mem://constraints/spec-scope`. Built-in 6-fixture in-memory `--self-test`. §25 advances Lovable 119 → 120, Cursor 118 → 119, Raw-LLM 113 → 115 (§25 reaches 120/120 ceiling for Lovable persona). Boundary quartet 3/4 closed. Prior: T-30 — slot 59 `check-no-ci-yaml-in-issues-folder.py`, gate #37.)
