...
**Version:** 4.51.0  
**Updated:** 2026-05-10 (Phase-5 G-6i / T-21 — slot 47 `check-ac-section-orphan-header.py` SHIPPED on disk (was paper-only with `**Status:** Active gate #28` since spec doc creation); gate #28 self-test + disk both load-proven (5/5 fixtures + 3 clauses across 7 §97 files = 255 ACs scanned, 0 violations); workflow step `§97 AC structural hygiene gate (#28 / T-21)` wired in `.github/workflows/spec-health.yml` directly after the audit-quoted-evidence-marker gate. Three load-bearing clauses: clause-1 no-orphan-ac (every `### AC-…` MUST be preceded by a `## ` parent earlier in the same file), clause-2 ac-id-uniqueness (no duplicate `### AC-…` IDs within a file), clause-3 section-name-uniqueness (no duplicate `## ` titles within a file). Status-tag presence + empty-parent-section detection were originally drafted as clauses 4 + 5; T-21 real-disk inspection found heterogeneous status-tag vocabularies (`[critical]`/`[high]`/`[medium]`/`[low]`/`[deferred]`/`[deprecated]` plus untagged) and intentionally-prose-only `## ` parents (Module Summary, Worked Examples, Cross-References) — both deferred to backlog ticket `ac-status-tag-and-parent-taxonomy-check` (T-22, slot TBD). R5 vacuous-pass: zero §97 files OR zero AC headers parsed → exit 1. **§28 §97 disk fix** (this turn) added `## v1.0 Core Acceptance Criteria (AC-28-01..AC-28-28)` parent header at line 11 — retired 28 orphan AC-28-NN headers in one shot, which is what unblocked clause-1 from passing real-disk. Phantom-script ledger **18 → 17** (-1 net; cumulative -17 across G-5 + G-6a..G-6i). `meta-verify-lockstep.py` still 0 violations; gates #23..#28 / #33/#34/#35 disk runs still 0 violations. Scorecard: §22/§23/§24/§25/§26/§27/§28 R-band C2 (Completeness) +1 each (seven §97 surfaces gain a machine-checked structural floor); §27 R-band C6 (Friction) +1 (one more backlog ticket closed; debug-routing exit codes enumerated). T-22 backlog ticket `ac-status-tag-and-parent-taxonomy-check` minted as the next §97-tightening turn (slot TBD).  
**Prior banner — Version:** 4.50.0; **Updated:** 2026-05-10 (Phase-5 G-6h — slot 46 `check-audit-quoted-evidence-marker.py` shipped; gate #27 self-test + disk both load-proven. Phantom-script ledger 19 → 18.)
