...
**Version:** 4.56.0  
**Updated:** 2026-05-10 (Phase-5 G-6l / T-24 — slot 53 `check-appshell-route-matrix.py` SHIPPED on disk; gate #31 self-test (6/6 fixtures) + disk (§24 §00 AS-NN matrix 8 rows × variant→behaviour binding 4 rows × invariant block 5 literals × AC-ADS-UI-04 surface 4 literals × Marketing AS-01/AS-02 both cite `MUST NOT import`+`AC-ADS-06`, 0 violations across 6 clauses) both load-proven on first run after one §24 §00 surgical fix (added `MUST NOT import \`AppShell\` (AC-ADS-06)` to AS-02 Notes column to satisfy clause-4 reverse-coverage). Workflow step `§24 AppShell route matrix gate (#31 / T-24)` wired in `.github/workflows/spec-health.yml` immediately after gate #30. Six clauses: clause-1 AS-NN matrix presence + ≥8 floor + AS-IDs unique + contiguous from AS-01; clause-2 variant enum closed-set `{Marketing, Console, Settings, Modal}` (or sentinel `(none — no shell)`); clause-3 variant↔binding bidirectional parity (every observed variant has binding row + ≥1 AS-NN cite, every binding row consumed by ≥1 AS-NN); clause-4 Marketing rows MUST cite `MUST NOT import` + `AC-ADS-06` (binds invariant-2 at matrix surface; sibling to gate #19 AST-side); clause-5 invariant block normative literal presence (`single source of truth`, `appshell-route-matrix-check`, `MUST NOT import from \`src/components/app/**\``, `--app-toolbar-height`, `5th variant` — clause-5 IS the slot 53 self-citation per Lesson #15); clause-6 AC-ADS-UI-04 surface literals (`AppShell route matrix`, `8-row AS-NN matrix`, `4-row variant→behaviour binding table`, `parity-locked`). Sibling chain: gate #19 (AST runtime side) + gate #31 (spec-side matrix) jointly form the AC-ADS-06 transitive boundary parity pair; the AppShell route matrix is now first-class navigation (paper-only for 15 cycles → load-proven). Closes oldest §24-side backlog ticket `appshell-route-matrix-check` (minted Phase-5 T-09). Promotes AC-ADS-UI-04 from conditional 20 to un-conditional 20 — last AC-ADS-UI-* family member to convert. R5 vacuous-pass: AS-NN matrix absent OR binding table absent OR AC-ADS-UI-04 surface absent → exit 1. Phantom-script ledger **15 → 14** (-1 net; cumulative -20 across G-5 + G-6a..G-6l). `meta-verify-lockstep.py` still 0 violations; gates #23..#31 / #33/#34/#35 disk runs still 0 violations. Scorecard: §24 R-band C3 (Testability) +1 (AppShell matrix mechanised); C5 (Implementability) +1 (AS-NN ↔ variant ↔ shell binding self-enforcing via gate #31 itself; cite-mechanism IS the gate name), C6 (Friction) +1 (last §24-side backlog ticket closed; route-matrix navigation now first-class) — §24 reaches 120/120 ceiling for Lovable+Cursor personas. §27 R-band C6 +1 (oldest §24-side backlog ticket retired).  
**Prior banner — Version:** 4.55.0; **Updated:** 2026-05-10 (Phase-5 G-6k / T-23 — slot 49 `check-ui-component-binding-matrix.py` shipped; gate #30 hard-fail self-test + disk; phantom 16 → 15; UI↔REST binding self-enforcing by construction.)
