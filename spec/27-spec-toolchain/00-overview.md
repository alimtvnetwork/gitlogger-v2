...
**Version:** 4.28.0  
**Updated:** 2026-05-10 (Phase-5 T-36 — P15 slot 64 `meta-verify-lockstep.py` shipped as gate #42. **First reflexive cross-gate meta-verifier.** 6-clause scan over `spec/27-spec-toolchain/NN-*.md` (active gate-bearing slots only) + §27 banner triple (`00-overview.md` / `98-changelog.md` / `99-consistency-report.md`): every gate enumerated in §27 §00 ↔ slot doc bijection (no orphans either side) / every active slot doc carries `## R5 — vacuously-passing scanner is auto-fail` + `vacuous-pass:` literal / every active slot doc declares `--self-test` with ≥6 fixtures + exactly one F-1 unique-passing / every active slot doc declares 4-row exit-code contract `0`/`1`/`2`/`3` / §27 banner triple gate-count integers agree (drift = clause-5 fail) AND §27 §00 carries literal `Self-enforcing via §27 backlog gate \`meta-verify-lockstep\`` (Lesson #15 reflexivity — first meta-gate must self-cite from the same surface it audits) / slot doc cites `mem://preferences/scorecard-ritual` + `mem://constraints/spec-scope`. Built-in 6-fixture in-memory `--self-test`. Closes §27-side cross-cohort meta-loop opened by gates #40 (§28 self-test) + #41 (§26 diagram parity). §27 advances Cursor 119 → 120 and Raw-LLM 117 → **120** — **§27 becomes the first folder to hit 120/120/120 triple-ceiling.** §26 + §28 each gain C4 +1 (cross-cohort meta-loop now proven closed bidirectionally). Prior: T-35 — P14 §26 final lift (AC-DG-01/02/06 literal-cited via gate #41).)
