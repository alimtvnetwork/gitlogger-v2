...
**Version:** 4.92.0  
**Version-prev:** 4.91.0  
**Updated:** 2026-05-11 (Sess-72 B-26 — created `00-mechanism-citation-index.md` v1.0.0 (~120-line audit-aid) inverting the cross-cohort `**Mechanically enforced by:**` citation graph: 11-row reverse index (slot → list-of-AC-callers across §22..§28), 8-row per-gate caller-multiplicity table, walker-pin carve-out (Lesson #15 reflexivity), drift-contract clauses D-1 (same-PR rule) / D-2 (coverage parity invariant) / R-3 (reverse-coverage invariant). Highest cross-cohort fan-out: gate #42 (5 callers across §25 + §26 + §27); cohort-local highest: gate #41 (4 callers in §26). Reduces walker blast-radius query cost ~25 KB → ~3 KB. AC-T-37 amended with **Callers reverse-index clause** extending four-way parity into a **fifth dimension — callers parity** (index row count, dedup, MUST equal `grep -rEn '\*\*Mechanically enforced by:\*\*' spec/22..28/`). §97 v2.23.0 → **v2.24.0** (AC count carried at 39 — body amendment only per Lesson #36). **Scorecard impact**: §27 R-band C4 (Consistency) **15 → 16** (band-anchor reached); §27 R total **107 → 108**; §27 cohort no longer Raw-LLM floor — ties §25 (R 108). **No** new active gate. **Total active gates: 26 unchanged.** B-26 status "open" → "closed"; remaining spec-only candidate B-28 only (projected 108 → ~109).  
**Prior banner — Version:** 4.91.0; **Updated:** 2026-05-11 (Sess-71 B-27 — walker-cost (KB) column appended to per-criterion scoreboard; §27 R C6 15 → 16 band-anchor; R total 106 → 107.)
