...
**Version:** 4.83.0  
**Updated:** 2026-05-11 (Sess-69 B-18 — initial Raw-LLM bottleneck decomposition `00-raw-llm-bottleneck-decomposition.md` shipped. Hand-scores §27 per-criterion Raw-LLM bands (C1=20, C2=20, C3≈12, C4≈11, C5=20, C6≈11; sum 94/120 ✓ matches cohort scorecard) and identifies five named lift candidates (B-19 inline red-green tests in 20 active-gate slots; B-16 navigation-quintet inline cross-ref; B-20 `check-slot-frontmatter-uniformity.py` candidate slot 66; B-21 2-tier slot index extension; B-22 `_carriers/` sub-namespace migration) with per-candidate cited self-enforcing mechanism + same-PR cohort-discipline coupling. Aggregate projection: 94 → ~110 if all five land. Pure spec-text audit aid; zero new gates, zero new ACs, zero implementation. Self-cited via gate #42 clause-5 + gate #39 link-target axis. Closes B-18 from Sess-68 remaining-tasks list.  
**Prior banner — Version:** 4.82.0; **Updated:** 2026-05-11 (Sess-68 B-8 — added AC-T-38 binding `00-tier1-bundle.md` Raw-LLM tier-1 partition manifest into §97 with line-budget invariant (≤ 2 500 lines), tier-promotion drift contract (5 clauses), per-persona pre-flight checklist contract, and reverse-coverage invariant (AC-T-* clauses MUST live in tier-1 §97 only). Mechanically enforced by gate #42 clause-5 banner-triple lockstep today + gate #34 (slot 35 audit-bundle-budget) extension in next backlog cycle. Completes the §27 navigation **quintet**: AC-T-30 + AC-T-31 + AC-T-36 + AC-T-37 + **AC-T-38**. §97 v2.19.0 → v2.20.0 (AC count 37 → 38). Lifts §27 C1 (Clarity) + C2 (Completeness) from 19 → 20 (cited mechanism: gate #42 clause-5 + reverse-coverage invariant + line-budget self-citation). Closes B-8 from Sess-67 remaining-tasks list. Pure spec-text surface; zero new code, zero new gates beyond the AC binding existing gate #42 to the new file. Prior: Sess-67 B-10 — AC-T-37 four-way parity contract.
