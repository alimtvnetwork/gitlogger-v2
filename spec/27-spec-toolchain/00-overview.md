...
**Version:** 4.49.0  
**Updated:** 2026-05-10 (Phase-5 G-6g — slot 45 `check-idempotency-observability.py` shipped; gate #26 self-test + disk both load-proven (6/6 fixtures + 5 clauses); workflow step `§22+§23+§24 idempotency observability gate` wired in `.github/workflows/spec-health.yml` directly after the seedable-config row gate. Five clauses: set-parity (Yes/No union of §23 R-1 + §24 S-2 columns reproduces AC-CAF-03 prose enumeration exactly — drift in either direction fails), observability-marker (AC-CAF-03 prose carries all four literals `EXPLAIN QUERY PLAN` + `IDENTICAL body` + `modulo TraceId` + `WE-4`), we4-cite (§23 §00 WE-4 carries `idempotent`/`200`/`DisconnectedAt` + canonical no-op phrasing + R-4 invariant 6 names R-07), ui-no-alias (§24 U-1 rows citing {R-01, R-04, R-08, R-15} MUST NOT contain literal `Idempotent` token), no-restate (§22 OpenAPI/observability + §25 audit findings MUST NOT inline parallel "Idempotency contract" matrix). R5 vacuous-pass: zero R-1/S-2 idempotency rows OR zero AC-CAF-03 anchors → exit 3 fixture-rot. **AC-CAF-03 (T-12)** promoted conditional 20 → load-proven 20 — fourth cross-cutting CAF gate after #23/#24/#25 (CAF-02/CAF-01/CAF-04). Single calibration: AC-CAF-03 declaring text in §24 §97 line 247 widened from GET-only Yes-set `{R-02, R-03, R-05, R-06, R-07, R-09, R-11, R-13}` to matrix-truth Yes-set `{R-02, R-03, R-05, R-06, R-07, R-09, R-10, R-11, R-12, R-13, R-14}` (PATCH endpoints R-10/R-12/R-14 already flagged Idempotent=Yes in S-2 — upsert-convergent per `INSERT … ON CONFLICT … DO UPDATE`; their omission from §97 was a §97↔§00 drift the gate uncovered on first contact). Phantom-script ledger **20 → 19** (-1 net; cumulative -15 across G-5 + G-6a..G-6g). `meta-verify-lockstep.py` still 0 violations; gates #23/#24/#25/#33/#34/#35 disk runs still 0 violations. Scorecard: §24 R-band C3 Testability +1 (U-1 no-alias contract + S-2 set-parity load-proven; §24 R-band 90 → 91); §27 R-band C6 Friction +1 (paper-only-since-T-12 ticket `idempotency-observability-check` — 7 cycles, second-oldest cross-cutting CAF backlog after CAF-04 — converts to load-proven; mirror of slot-44 G-6f T-08 backlog conversion).
