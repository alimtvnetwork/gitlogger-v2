...
**Version:** 4.78.0  
**Updated:** 2026-05-11 (Sess-67 G-8 — §26 R-band lift via `linter-scripts/check-diagram-parity.py` clause-6 `narrative-header-schema`. Frozen `NARRATIVE_HEADER_KEYS = ('Diagram type:','What this answers:','Authoritative source:','Audience:')` + 15-token `MERMAID_DIRECTIVES` enum scan every active `.mmd` for the 4 keys in canonical order before the first directive line; YAML frontmatter tolerated; intervening `%%` comments + blank lines permitted. `--self-test` 6/6 → 8/8 (F-7 `Audience` stripped → clause-6 fail; F-8 keys swapped → clause-6 order fail). Mid-flight `_make_fixture` refactor regression repaired by restoring the `consumes-binding-completeness` + `er-entity-superset` arms in `run_against`; F-2/F-3 green again. Live disk: `OK: §26 diagram parity gate clean (--check=all)`. §26 §97 AC-DG-23 gained `**Mechanically enforced by:** … clause-6` paragraph pinning constant↔AC reflexivity (Lesson #36). Scorecard: §26 R-band C3 +1 (clause-6 self-test = second self-citing mechanism alongside gate #28 §97 hygiene), C5 +1; §27 C6 +1 (G-8 retired).  
**Prior banner — Version:** 4.77.0; **Updated:** 2026-05-11 (Sess-67 G-T-22 — slot 47 deferred clauses 4+5 shipped, T-22 backlog ticket retired.)
