...
**Version:** 4.76.0  
**Updated:** 2026-05-11 (Sess-67 G-6z — perimeter cleanup sweep on `STRUCTURAL_EXEMPT_FILES`. Audited all 8 entries: 7 retained (each carries ≥2 intentional Markdown links to canonical upstream contract surfaces whose link-text disambiguates the reference; per-link disposition documented inline in `linter-scripts/check-no-out-of-scope-spec-folder-link.py`). 1 removed: `spec/28-universal-ci-cli/05-config-resolution.md` had a single isolated `[spec/13-generic-cli §97 AC-22](../13-generic-cli/97-acceptance-criteria.md)` Markdown link, converted to backticked path-token cite + adjacency-marked prose ("**out-of-scope** per the locked-7 perimeter"; "audited via §27 perimeter gate #39"). Live disk: gate #39 `OK (mode=all, files=175, exemptions=169)`. `--self-test` 6/6 unchanged. New audit-protocol comment block in the constant declaration requires Sess-46-style per-link disposition for any future addition to the exempt list. Closes G-6z; perimeter-exempt surface area shrunk 8 → 7 (-12.5%) without weakening the gate.  
**Prior banner — Version:** 4.75.0; **Updated:** 2026-05-11 (Sess-67 G-6w-mirror — §22 `18-schema.sql` AppLink rebased to §23 contract shape; gate #22 walks both surfaces via `--all-sources`)
