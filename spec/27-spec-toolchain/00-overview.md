...
**Version:** 4.22.0  
**Updated:** 2026-05-10 (Phase-5 T-30 — added slot 59 `check-no-ci-yaml-in-issues-folder.py`, gate #37. **Second of 4 boundary-quartet gates** mechanising `mem://constraints/spec-scope` on disk. Closes §27 backlog `no-ci-yaml-in-issues-folder-check` minted Phase-5 T-09 (paper-only for 19 cycles). 6-clause §25 no-CI-yaml boundary scan: no ` ```yaml `/` ```yml ` fence with CI-workflow shape tokens (`runs-on:`/`uses: actions/`/`jobs:`/`on: push|pull_request|schedule`) in §25 `.md` — plain non-CI YAML PERMITTED / no `spec/25-…/(workflows|ci|github)/*.yml` ownership claim / no bare CI keywords in unfenced prose / AC-finding cross-refs to §27 gate names inside `### AC-AI-*` backtick spans EXEMPT (positive guarantee F-5 regression) / §25 §00 boundary declaration `CI-workflow YAML is owned exclusively by §27` + Lesson #15 gate self-citation / slot doc cites `mem://constraints/spec-scope`. Built-in 6-fixture in-memory `--self-test`. §25 advances Lovable 118 → 119, Cursor 116 → 118, Raw-LLM 111 → 113. Boundary quartet 2/4 closed. Prior: T-29 — slot 58 `check-no-sql-ddl-in-ui-folder.py`, gate #36.)  
