...
**Version:** 4.77.0  
**Updated:** 2026-05-11 (Sess-67 G-T-22 — closed §27 backlog ticket `ac-status-tag-and-parent-taxonomy-check` (the deferred T-22 item from T-21). Extended `linter-scripts/check-ac-section-orphan-header.py` (slot 47, gate #28) with two new clauses bound to frozen on-disk constants: clause-4 `status-tag-vocabulary` validates any opt-in `` `[xxx]` `` tag against `STATUS_TAG_VOCABULARY = ('active','critical','high','medium','low','deferred','deprecated','draft')` (8-tag set derived from full real-disk survey of all 7 §97 files); clause-5 `empty-parent-taxonomy` rejects any `## ` parent with zero `### AC-…` children unless its title is on `EMPTY_PARENT_ALLOWLIST` (13 titles: Format, Module Summary, Inlined Contracts, Worked Examples, Cross-References, Purpose, Notes, Legacy Index, Slot Delegation Map, AC Family Prefix Index, Per-artifact criteria, Validation, Test Invariant Index — prefix-match for parenthetical-suffix variants like "Slot Delegation Map (Phase 153 Task A24-fu6)"). `--self-test` 5/5 → 8/8 (added F-6 unknown-tag, F-7 non-allowlist empty parent, F-8 happy-path with allowlist-parent + canonical-tag). Live disk: `OK — 7 §97 file(s), 255 ACs, check=all` — both new clauses pass on real spec corpus, validating the vocabularies are complete. Spec doc §47 deferred-to-T-22 caveat replaced with the new clause-4/5 contract + spec-doc ↔ constant binding rule (Lesson #15 reflexivity: any new vocabulary entry without a §47 spec-doc row fails the gate next CI cycle). CHECKS tuple grew `('all','no-orphan-ac','ac-id-uniqueness','section-name-uniqueness','status-tag-vocabulary','empty-parent-taxonomy')`. Closes G-T-22; T-22 backlog ticket retired. Scorecard: §22/§23/§24/§25/§26/§28 R-band C2 +1 each (machine-checked status-tag + parent-taxonomy floor across all 7 §97 surfaces); §27 R-band C5 +1 (frozen-vocabulary constant binds spec-doc ↔ script ↔ disk in 3-way reflexive loop), C6 +1 (T-22 backlog ticket — last toolchain-side carry-over from T-21 — retired).  
**Prior banner — Version:** 4.76.0; **Updated:** 2026-05-11 (Sess-67 G-6z — perimeter cleanup sweep; `STRUCTURAL_EXEMPT_FILES` 8 → 7.)
