...
| 21 | `applink-xor-clause-check` | Active | §27 | `python3 linter-scripts/check-applink-xor-clause.py --check all --self-test` | 0 = pass; 1 = §23 AppLink DDL XOR-CHECK clause / disconnect-CHECK / locked-ID seed / partial-index parity violation (AC-ADB-05 + AC-ADB-13 + AC-ADB-R-4 invariant 6); 2 = invocation error; 3 = fixture-rot (synthetic source missing) | validate stage |
