...
**Version:** 4.67.0  
**Updated:** 2026-05-11 (Sess-67 G-6w — slot 39 `check-applink-xor-clause.py` phantom-cleared (gate #22 was citation-only since Phase-5 T-15). Shipped a 230-LOC scanner walking spec/23-app-database/00-overview.md with 4 clauses + R5 vacuous-pass anchor: clause-1 XOR CHECK with two SELECT-discriminator disjuncts joined by OR (rejects AND/3rd-disjunct via inter-disjunct bridge regex); clause-2 disconnect-invariant CHECK both disjuncts present; clause-3 locked-ID seed `INSERT OR IGNORE INTO AppLinkType(AppLinkTypeId, Name) VALUES (1,'GitProfile'),(2,'Repo')` — bare seed without explicit IDs flagged per AC-ADB-13/T-10/WE-2; clause-4 partial indexes IX_AppLink_TargetRepoId / _TargetGitProfileId with `WHERE Target… IS NOT NULL` on both lanes. `--self-test` 6/6 fixtures green (F-1 unique-pass + F-2 XOR→AND + F-3 bare seed + F-4 IX WHERE stripped + F-5 disconnect CHECK absent + F-6 R5 empty corpus). Live disk run on §23 §00: **clean on first run** — AC-ADB-05/13 + WE-2/3/4 are now load-proven, not paper. §22 `18-schema.sql` mirror coverage deferred (G-6w-mirror) — §22 carries the pre-rename `GitProfileId`/`RepoId` column shape; rebase to `Target…Id` is a separate work item. Wired in `.github/workflows/spec-health.yml` as step "§23 AppLink XOR clause gate (#22 / G-6w / slot 39)" immediately after gate #46 with `--self-test` + live disk run. Gate-ledger I-1 EXISTS failures **2 → 1**; I-2 WIRED phantoms **3 → 2** (gate #22 now resolves to a real disk script). `meta-verify-lockstep.py` still 0 violations; gates #22..#46 disk runs all green. Scorecard: §23 R-band C3 (Testability) +1 + C5 (Implementability) +1 (AppLink polymorphic resolution + disconnect-invariant + locked-ID seed + partial-index quartet now self-enforcing — cite mechanism IS gate #22 itself); §27 R-band C4 (Consistency) +1 (one more I-2 phantom cleared).  
**Prior banner — Version:** 4.65.0; **Updated:** 2026-05-11 (Sess-67 G-6u/G-6v — phantom-citation cleanup waves 1+2; I-1 EXISTS failures 14 → 2; banner-triple still 26.)
