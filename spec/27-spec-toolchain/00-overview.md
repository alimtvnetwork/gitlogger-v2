...
**Version:** 4.75.0  
**Updated:** 2026-05-11 (Sess-67 G-6w-mirror — §22 `18-schema.sql` AppLink rebased to the §23 contract shape (TargetGitProfileId/TargetRepoId NULL columns + IsActive INTEGER NOT NULL + DisconnectedAt INTEGER NULL + two CHECKs (XOR-with-SELECT-discriminator + disconnect-invariant) + four indexes including the partial Target indexes); pre-rename `GitProfileId`/`RepoId` columns + bare-OR CHECK + flat `IxAppLinkApp` index removed in lockstep. Gate #22 (slot 39 `check-applink-xor-clause.py`) extended with `MIRROR_SOURCES = (§23 §00, §22 §18-schema.sql)` lockstep tuple + `--all-sources` flag that walks every entry and fails on the first divergence; CI step `§23 AppLink XOR clause gate` now runs `--self-test` + `--all-sources` (was `--self-test` + single-source live). Live disk: `OK on §23 §00` + `OK on §22 §18-schema.sql` (both 4-clause clean). `--self-test` 6/6 unchanged. Mirror-coverage promoted from "deferred since G-6w" to "load-proven across both surfaces simultaneously". Closes the §22-side half of G-6w; gate #22 is now the **first** §22-touching gate that scans BOTH the §23 contract AND the §22 mirror in a single CI step. `meta-verify-lockstep.py` 0 violations; gate-ledger 3-invariants still all-pass; banner-triple still 26. Scorecard: §22 R-band C3 +1 (mirror surface load-proven) + C5 +1 (lockstep-with-§23 anchored on disk by `MIRROR_SOURCES`); §23 R-band carried at 120 ceiling; §27 R-band C2 +1 (`--all-sources` + frozen MIRROR_SOURCES tuple pattern), C5 +1 (gate-script ↔ workflow ↔ both spec sources 4-way bind).  
**Prior banner — Version:** 4.73.0; **Updated:** 2026-05-11 (Sess-67 G-6aa — frozen RETIRED_GATE_NUMBERS; I-3 NUMBERED 20-gap → 0; gate-ledger fully green.)
