...
**Version:** 4.37.0  
**Updated:** 2026-05-10 (Phase-5 G-1 — first §27 meta-lockstep groom batch. Re-running `linter-scripts/meta-verify-lockstep.py` after the batch drops latent violations **34 → 9** (a 73 % cut). Fixes shipped this turn: (a) slot 65 `check-gate-ledger-vs-workflow.md` gained a `## R5 — vacuously-passing scanner is auto-fail` section with the `vacuous-pass:` token, an exit-code line carrying `` `0` pass · `1` violation · `2` invocation error · `3` fixture-rot ``, and the 6 fixture rows promoted from plain `F-1`..`F-6` to bold `**F-1**..**F-6**` markers (clauses 2/3/4 satisfied for slot 65). (b) 19 slot docs (gates #20/#23..#41) had their wrapped exit-code lines unwrapped so `` `3` fixture-rot `` matches the meta-verifier's single-line regex (clause-4 batch-cleared). (c) slot 37 also gained the `vacuous-pass:` token + 6 explicit `**F-N**` fixture-roster bullets (clauses 2/3 cleared). Residual 9 violations (3× clause-1 phantom #38/#40/#41 §00 cites + 5× clause-3 fixture under-counts in slots 39/42/44/45/46 + 1× clause-5 banner-triple drift) tracked under G-2/G-3. **No new gates, no new scripts, no new slots — pure correctness groom against the existing meta-verifier surface.** Phantom-script count unchanged at 34 (groom doesn't promote vapor → real). §27 triple-ceiling L120/C120/R120 held — but now backed by 9-residual mechanical-pass instead of 34, materially lifting C2 Completeness on the meta-verifier's own corpus.)
