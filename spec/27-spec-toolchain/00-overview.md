...
**Version:** 4.57.0  
**Updated:** 2026-05-10 (Phase-5 G-6m / T-25 — slot 54 `check-seed-id-explicit-locked-form.py` SHIPPED on disk; gate #32 self-test (6/6 fixtures: happy-path + primary-omits-AppStatusId + reference-bare-do-nothing + matrix-missing-row + forbidden-block-removed + cite-chain-broken) + disk (§23 §00 seed-data section: PRIMARY lane 2 explicit-PK INSERTs × REFERENCE lane 2 parity-pinned `ON CONFLICT (<pk>) DO NOTHING` × parity matrix 5 rows in fixed order × forbidden-shape block 3 literals + ❌/⚠/❌ icons × cite-chain 8 anchors, 0 violations across 6 clauses) both load-proven on first run after one §23 §00 surgical fix (added `XOR target invariant (AC-ADB-05)` + `AC-ADB-11` cite-chain anchors to the seed-data section span line 221). Workflow step `§23 seed-ID explicit locked-form gate (#32 / T-25)` wired in `.github/workflows/spec-health.yml` immediately after gate #31. Six clauses: clause-1 PRIMARY-lane explicit-PK literals (AppStatusId tuples 1/2/3 LOCKED + AppLinkTypeId tuples 1/2 LOCKED — IDs are AC-ADB-13 contract); clause-2 REFERENCE-lane parity-pinned form (`ON CONFLICT (app_status_id) DO NOTHING` + `ON CONFLICT (app_link_type_id) DO NOTHING` — explicit PK conflict-target REQUIRED, no bare `DO NOTHING`); clause-3 parity-matrix bidirectional 5-row coverage (AppLinkType 1/2 + AppStatus 1/2/3 fixed order; matrix subset-superset against fence tuples); clause-4 forbidden-shape block literal pinning (`silently de-anchors` load-bearing semantic + `IDs are the contract` AC-ADB-13 normative phrase + 3 numbered items + ❌/⚠/❌ icons in order); clause-5 cite-chain 8 anchors (`AC-ADB-13`, `AC-ADB-11`, `T-10 remediation`, `MATERIALISE`, `DO NOT MATERIALISE`, `XOR target invariant`, `Discriminator`, `Q1` — clause-5 IS the slot 54 self-citation per Lesson #15); clause-6 no-restate of locked-ID seed in §22/§24/§25 SQL fences (Lesson #36 link-don't-restate). Sibling chain: gate #22 AppLink CHECK clause-2 depends on these locked IDs; gate #33 (slot 55 dialect-precedence-banner) cites the same `MATERIALISE`/`DO NOT MATERIALISE` markers — together they form the seed-shape ↔ dialect-banner parity pair. Closes §27 backlog ticket `seed-id-explicit-locked-form-check` minted T-10 (paper-only for 13 cycles → load-proven). Promotes AC-ADB-13 from conditional 20 to un-conditional 20. R5 vacuous-pass: seed-data section absent OR PRIMARY/REFERENCE fence absent OR parity matrix absent OR forbidden block absent → exit 1. Phantom-script ledger **14 → 13** (-1 net; cumulative -21 across G-5 + G-6a..G-6m). `meta-verify-lockstep.py` still 0 violations; gates #23..#32 / #33/#34/#35 disk runs still 0 violations. Scorecard: §23 R-band C3 (Testability) +1 (seed-ID locked-form mechanised end-to-end), C5 (Implementability) +1 (PRIMARY-lane explicit-PK + REFERENCE-lane conflict-target self-enforcing via gate #32 itself; cite-mechanism IS the gate name), C6 (Friction) +1 (oldest §23-side backlog ticket T-10 retired) — §23 R-band lifts toward ceiling. §27 R-band C6 +1 (oldest §23-side backlog ticket retired).  
**Prior banner — Version:** 4.56.0; **Updated:** 2026-05-10 (Phase-5 G-6l / T-24 — slot 53 `check-appshell-route-matrix.py` shipped; gate #31; phantom 15 → 14; oldest §24-side backlog ticket retired.)
