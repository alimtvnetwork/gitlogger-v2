...
**Version:** 4.16.0  
**Updated:** 2026-05-10 (Phase-5 T-24 — added slot 53 `check-appshell-route-matrix.py`, gate #31. First §24-route-surface gate; closes §27 backlog `appshell-route-matrix-check` minted T-09 — oldest §24-side backlog ticket (paper-only for 15 cycles). 6-clause cross-table scan over §24 §00 AppShell Route Matrix (line 263) + variant→behaviour binding table (line 283) + invariants 1-4 (line 290) + §24 §97 AC-ADS-UI-04 (line 303): matrix shape ≥8 contiguous AS-NN rows / AppShellVariant closed-set discipline / variant→behaviour binding parity bidirectional / Marketing AS-NN rows cite `AC-ADS-06` + `MUST NOT import` / invariant block self-citation per Lesson #15 / AC-ADS-UI-04 surface + literal presence + `[active]` status tag. Built-in 6-fixture in-memory `--self-test`. Promotes AC-ADS-UI-04 from conditional 20 to un-conditional 20 — last AC-ADS-UI-* family member to convert. §24 reaches 120/120 ceiling for Lovable+Cursor personas; Raw-LLM 114 → 115. Prior: T-23 — slot 49 `check-ui-component-binding-matrix.py`, gate #30.)  
