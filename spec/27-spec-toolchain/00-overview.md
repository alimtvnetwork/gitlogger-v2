...
**Version:** 4.25.0  
**Updated:** 2026-05-10 (Phase-5 T-33 — added slot 62 `check-ci-cli-self-test-harness.py`, gate #40. **First cross-cohort gate** enforcing §27 self-test pattern outside §27 — first §28 floor-lift turn. 6-clause §28 self-test harness contract scan: §04 declares `--self-test` flag with 4 semantic markers (`built-in`/`no network`/`no real CI provider`/`no real git repo`) / §00 or §97 carries R5 vacuously-passing inheritance literal / per-`--check` mode fixture coverage in §97 AC-28-* family (≥6 fixtures, F-1 unique passing) / §07 declares 4-row exit-code table including `3 → fixture-rot` / §28 §00 harness declaration `glci ships a built-in --self-test harness` + Lesson #15 gate self-citation / slot doc cites `mem://preferences/scorecard-ritual` + AC-28-49 placeholder. Built-in 6-fixture in-memory `--self-test`. §28 advances Lovable 117 → 119, Cursor 117 → 119, Raw-LLM 114 → 116. §27 C4 +1 (self-test discipline bidirectionally enforced). Prior: T-32 — slot 61 `check-no-out-of-scope-spec-folder-link.py`, gate #39.)
