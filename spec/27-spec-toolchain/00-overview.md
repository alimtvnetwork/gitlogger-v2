...
**Version:** 4.58.0  
**Updated:** 2026-05-10 (Phase-5 G-6n / T-29 — slot 58 `check-no-sql-ddl-in-ui-folder.py` SHIPPED on disk; gate #36 self-test (6/6 fixtures: complete-clean + sql-fence + bare-ddl + migration-path + binding-table-exemption-broken + boundary-stripped) + disk (5 clauses across 4 §24 .md files: 0 SQL fences, 0 bare-DDL keywords in unfenced prose, 0 §24-owned migration paths, U-1/U-3 column-name exemption exercised, §24 §00 boundary declaration + gate self-citation present) both green on FIRST run after one §24 §00 surgical insert at line 158 (replaced solo `---` separator with `> **No-DDL boundary (Normative — Phase-5 T-29):** **App-side DDL is owned exclusively by §23.** … **Self-enforcing via §27 backlog gate \`no-sql-ddl-in-ui-folder-check\`** (slot 58 / gate #36 / Lesson #15 reflexivity pin)`). Workflow step `§24 no-DDL boundary gate (#36 / T-29)` wired in `.github/workflows/spec-health.yml` immediately after gate #32. Five clauses: clause-1 no SQL fences (8 forbidden langs `{sql, sqlite, postgres, pg, mysql, mariadb, ddl, plpgsql}` — fence regex requires line start with ≤3 leading spaces + ``` + lang token, so inline backticked-mention of ` ```sql ` in §97 prose is exempt by-construction); clause-2 no bare DDL keywords (`CREATE TABLE `/`ALTER TABLE `/`DROP TABLE `/`CREATE INDEX `/`ALTER COLUMN `) in unfenced non-backtick-span prose; clause-3 no §24-owned `migrations/`/`schema/`/`ddl/` `.sql` path references; clause-4 R5 vacuous-pass anchor — ≥1 U-1/U-3 binding-table row MUST mention a DDL column name (`IsActive`, `AppId`, `AppLinkType`, `AppStatusId`) so clause-4's column-name exemption logic is provably wired; clause-5 §24 §00 boundary block literal pinning + gate self-citation per Lesson #15. Sibling chain: gate #19 (AST runtime side: `Marketing`-variant routes MUST NOT import `src/components/app/**`) + gate #31 (spec-side AS-NN matrix surface) + gate #36 (no-DDL spec-side surface) jointly form the §24-as-render-only triangle — every escape hatch (component import, route matrix cite, DDL declaration) closed by independent machine check. Closes §27 backlog `no-sql-ddl-in-ui-folder-check` (minted Phase-5 T-09; paper-only multi-cycle). R5 vacuous-pass: zero §24 .md files OR zero U-1/U-3 column-name rows OR boundary literals absent → exit 1. Phantom-script ledger **13 → 12** (-1 net; cumulative -22 across G-5 + G-6a..G-6n). `meta-verify-lockstep.py` still 0 violations; gates #23..#32 / #33/#34/#35/#36 disk runs all green. Scorecard: §24 R-band C5 (Implementability) carried at ceiling; C6 (Friction) carried (no-DDL boundary now self-enforcing — closes backlog T-09 sibling). §23 R-band C4 (Consistency) +1 (single-source-of-truth-for-DDL invariant gains its first cross-folder boundary gate). §27 R-band C6 +1 (one more backlog ticket retired).  
**Prior banner — Version:** 4.57.0; **Updated:** 2026-05-10 (Phase-5 G-6m / T-25 — slot 54 `check-seed-id-explicit-locked-form.py` shipped; gate #32; phantom 14 → 13; oldest §23-side backlog ticket retired.)
