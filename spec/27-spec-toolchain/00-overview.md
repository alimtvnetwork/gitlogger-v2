...
**Version:** 4.45.0  
**Updated:** 2026-05-10 (Phase-5 G-6c-followup — gate #33 disk run flipped warn-only → HARD-FAIL; all 14 clause-5 unlabelled-fence violations resolved by per-fence `**PRIMARY lane (SQLite — MATERIALISE):**` markers in §23 §00 (13 fences) + §23 §97 (1 fence) + REFERENCE-lane re-banner of the §23 §00 PostgreSQL appendix RLS-policies fence; `python3 linter-scripts/check-dialect-precedence-banner-present.py` → `OK (mode=all)`; gates #24/#34/#35 still 0 violations; phantom ledger unchanged at 22 (no new gate). Scorecard: §23 R-band Cursor 120 carried; §23 R-band Raw-LLM **119 → 120** (C3 Testability +1 — clause-5 disk-side reconciliation lifts the last gate-#33 conditional surface to load-proven; §23 joins §27/§28 at triple-ceiling 120/120/120 in the Raw-LLM persona). §22 unchanged at v4.47.0 lift. §27 R-band unchanged. (13 in §23 §00, 1 in §23 §97); workflow step "§22+§23 dialect-precedence banner gate" wired in `.github/workflows/spec-health.yml` directly after gate #24. Implements 6-clause cross-folder scan: clause-1 §23 §00 precedence-heading position (BEFORE first SQL fence), clause-2 lane-table closed-set discipline (PRIMARY+REFERENCE rows; PRIMARY literals `**PRIMARY**`/`SQLite`/`PascalCase`/`INTEGER`/`ACTIVE`/`✅ YES`; REFERENCE literals `**REFERENCE**`/`PostgreSQL`/`snake_case`/`boolean`/`REFERENCE ONLY`/`❌ NO`/`silent dialect-flip is FORBIDDEN`; ordering enforced), clause-3 cross-cuts pin literals (`Boolean policy (AC-ADB-11`+`Timestamp parity (AC-ADB-16)`+`Seed ID parity (AC-ADB-13)`+`Any code emitting \`boolean\` on the App database is a violation`+`EXTRACT(EPOCH FROM`+`INSERT … VALUES (1,'GitProfile'),(2,'Repo')`), clause-4 AI-walker contract literals (`partial-context violation`+`re-anchor`+`§00 Quick-Nav guarantees this pin is reached on a TOC walk`), clause-5 per-fence lane marker (4-form whitelist: `**PRIMARY lane (SQLite — MATERIALISE):**` / `**REFERENCE lane (PostgreSQL — DO NOT MATERIALISE; shown for parity audit only):**` / `### 🚫 REFERENCE-ONLY` / `> **⚠️ Reference / Secondary dialect (per AC-ADB-11).**` matched within 30-line lookback; `_archive/` exempt), clause-6 §22 §00 single-dialect row (`Database engine` paired with `SQLite` within 5 lines). **G-6c-followup minted**: §23 §00 carries multiple long DDL bodies (e.g. AppLink at line 172 — 30 lines of column declarations push the PRIMARY-lane marker at line 217 outside the 30-line lookback for fences at line 320/340/350/357 and the long REFERENCE block at line 859 pushes its `### 🚫 REFERENCE-ONLY` header outside lookback for fences at line 951). Followup will either tighten lookback to per-section, restate the lane marker per-fence, or split DDL bodies — same pattern as G-6a → G-6a-followup. Phantom-script ledger 23 → 22. `meta-verify-lockstep.py` still 0 violations. Gates #24/#34/#35 disk runs still 0 violations. Scorecard: §23 R-band Cursor 120 carried; §23 R-band Raw-LLM **118 → 119** (C3 Testability +1 — banner position + lane-table + cross-cuts + AI-walker literals load-proven via gate #33 self-test; clause-5 disk pending). §22 R-band C4 Consistency +1 (clause-6 single-dialect declaration mechanically pinned). §27 R-band C6 Friction +1 (paper-only `dialect-precedence-banner-present` ticket — 14 cycles since T-08 — converts to load-proven via self-test; full disk lift pending G-6c-followup).
