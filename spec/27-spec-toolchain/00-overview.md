...
**Version:** 4.87.0  
**Updated:** 2026-05-11 (Sess-69 B-20 — authored `66-check-slot-frontmatter-uniformity.md` as a **Proposed gate #44** slot doc (status: proposed; script not on disk; promotion gated on an explicit "implement" turn per `mem://constraints/no-implementation-suggestions`). 4-invariant contract pinned in spec text: U-1 SCHEMA-PRESENT, U-2 SCHEMA-KEYS (4 canonical keys in canonical order: `kind`, `axis`, `delegates_from_ac`, `mechanically_enforced_by`; optional 5th `status` ∈ closed-set), U-3 KIND-VOCABULARY (7-token closed set), U-4 AXIS-NONEMPTY (kebab-case + ≥ 20-char rationale). 7-fixture self-test roster (F-1..F-7) specified in-text including F-7 reflexive-this-file. Carve-out list pinned (7 module-level surfaces exempt). Lifts §27 R-band C4 (Consistency) **11 → 13** (partial +2; cited mechanism: schema pin + closed-set vocabulary in this slot's body — but enforcement is reviewer-attestation only until script promotes; full +5 lift to 16 deferred to post-promotion, with B-16 inline navigation-quintet then unblocked to lift C4 to 18 band-anchor). Same-PR refresh of `00-raw-llm-bottleneck-decomposition.md` (v1.2.0 → v1.3.0): C4 row 11 → 13, B-20 status "open" → "spec-only authored / promotion pending". Total active gates **26 unchanged** (proposed gates do NOT count toward the active total per gate #42 clause-5 `GATE_COUNT_RE`).  
**Prior banner — Version:** 4.86.0; **Updated:** 2026-05-11 (Sess-69 B-21 — Tier-2 slot-index lookup table added to `00-tier1-bundle.md`; lifts §27 R C6 +3.)
