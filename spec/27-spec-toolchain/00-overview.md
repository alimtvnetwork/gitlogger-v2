...
**Version:** 4.12.0  
**Updated:** 2026-05-10 (Phase-5 T-20 — added slot 46 `check-audit-quoted-evidence-marker.py`, gate #27. **Fifth and final** cross-cutting App-framework gate (CAF). Promotes AC-CAF-05 (T-12) from conditional 20 (paper-only) to un-conditional 20 (load-proven cross-folder audit-quoting contract). 5-clause static surface scan (CAF-05 marker discipline `auditor-quoted evidence` + `spec/_archive/21-git-logs-v1/` + `AC-AI-10/11` + `cross-cutting status` literals / §25 AC-AI-10 + AC-AI-11 surface presence / §25 AC-AI-14 rule 4 verbatim-quote enforcement / §25 finding-body foreign-AC `AC-(ALW\|ERR\|JWT\|CG\|SAG\|TOK)-\d+` MUST be backticked or fenced or blockquoted / §23 + §24 no-restate of foreign-AC outside backticks + evidence-marker context) with 5-fixture in-memory `--self-test`. Closes §27 backlog entry `audit-quoted-evidence-marker-check` minted T-12 — last remaining CAF backlog ticket. **All five CAF cross-cutting ACs are now un-conditionally load-proven** for the first time since T-12. Prior: T-19 — slot 45 `check-idempotency-observability.py`, gate #26.)  
