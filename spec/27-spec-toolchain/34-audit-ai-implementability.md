...
### AC-34-20 — Module-level `content_axis` MUST cascade to every chunk; per-chunk axis re-detection forbidden `[critical]`
- **Given** AC-34-12 requires every module's root `00-overview.md` to declare a single `content_axis` from the 5-value enum, AND AC-34-15..19 introduced multi-chunk re-scoring where each chunk is independently submitted to the gateway, AND a Phase 154 cache audit observed `spec/22-git-logs-v2` (declared `axis: normative-contract`) emitting 3 chunks with `per_chunk_v7[1].axis == "audit-corpus"` + `per_chunk_v7[2].axis == "audit-corpus"` (see `.lovable/cache/audit-ai/22-git-logs-v2.json`) — the LLM re-detected the chunk axis from the slice content (test plans, AC detail files, CLI fixtures) and the merger applied audit-corpus weight multipliers to those chunks, dragging the weighted total from 84 raw → **73.1 weighted (GOOD band)** despite the module's binding contract surface being normative; AND `spec/25-app-issues` exhibits the inverse symmetry (declared `audit-corpus`, but the per-chunk re-detection compounds the weighting penalty rather than letting the module-declared axis bind),
- **When** `audit_module()` per AC-34-17 invokes `call_gateway()` for each chunk dict produced by `pack_chunks()`, AND when `merge_chunk_scores()` per AC-34-15(d) folds per-chunk scores into the module total,
- **Then** (a) the gateway prompt for EVERY chunk MUST pin `axis = root_module_axis` (read from `mod_dir / "00-overview.md"` front-matter `axis:` or `content_axis:` field, validated per AC-34-12) with explicit prompt language `"This chunk is part of a multi-chunk module whose declared content_axis is <X>. Score against the <X> axis multipliers; do NOT re-detect axis from chunk content."`; (b) `parse_score()` MUST overwrite any LLM-returned `axis` field with the module-level value before returning the parsed dict — defensive belt-and-braces against prompt non-compliance; (c) `merge_chunk_scores()` MUST refuse to merge when any input chunk's `axis` differs from the module-level axis (raise `ValueError("axis cascade violation: chunk N axis=<Y>, module axis=<X>")`) — fails CI loud rather than silently producing the audit-corpus drag observed in §22; (d) the cache JSON `per_chunk_v7[i].axis` field MUST equal the module-level axis for ALL chunks of a single module — `linter-scripts/test/test-axis-cascade.sh` MUST assert this invariant on every cached module via `jq -e '[.per_chunk_v7[].axis] | unique | length == 1'`; (e) **expected lift on landing** (deferred until gateway credits restore — HTTP 402 at landing per Lesson #86 oscillation): `spec/22-git-logs-v2` 73.1 → ≥84 weighted (chunks 2+3 reclassified normative-contract, removing the audit-corpus multiplier penalty on D4+D5); `spec/25-app-issues` 93.0 → confirm-no-regression (already declares audit-corpus, so cascade is a no-op there, but the assertion catches future drift).
- **Verifies:** §34 axis-cascade contract — closes the per-chunk axis re-detection class identified during the Phase 154 §22 cache audit. Mirror of AC-34-12 (axis declared once per module) extended to the multi-chunk boundary. Self-tested at `linter-scripts/test/test-axis-cascade.sh` (3 assertions: §22 chunks all-equal axis post-rescore; §25 chunks all-equal axis; synthetic axis-mismatch input raises `ValueError`). **Lesson #88** (codified at AC-34-20 ship): when LLM auditors are given a chunk slice without the binding axis context, they default to per-slice content classification (a test-plan slice "looks like" audit-corpus to the LLM regardless of the parent module's declared contract role) — the durable fix is at the prompt + merger boundary (this AC), NOT at the slice-content boundary (which would require re-architecting the module). Mirror of Lesson #29 (auditor-misreading pin) at the multi-chunk axis-detection axis.
