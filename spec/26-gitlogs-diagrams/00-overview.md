...
**Version:** 3.13.0
**Updated:** 2026-05-11 (Sess-84 B-2-§26 — banner-triple lockstep with §97 v3.9.0 → **v3.10.0** gate-citation matrix mint. Closed-set "Mechanically enforced by — gate-citation matrix" inserted between Inlined Contracts and Acceptance Criteria, mapping all 29 AC families (24 AC-DG-NN + 5 cross-ref AC-NN) to their auditing §27 gate slot. Coverage 29/29 (100%). Slot `63-check-diagram-parity.md` cited 17× (59% — primary §26 diagram-parity auditor; reflects §26 IS the diagram cohort). 4-clause reflexive drift contract pinned. Sister to §27 cross-cohort DAG edge **E-7**. Mirror of §22/§23/§24 §97 closures (Sess-83/81/82) — completes the B-2 mirror-quartet for the citation-matrix pattern. AC count unchanged at 24 AC-DG; no AC body edits. **Scorecard impact**: §26 R-band C4 (Consistency) carried at 20 — defensibility hardened by citation-closure mechanism now sister to §22/§23/§24 closures (mirror-quartet anchor for citation-matrix pattern joins the existing mirror-septet anchor for walker-cost-reflexivity pattern; §26 is the only cohort cited in BOTH cross-cohort mirror-anchor closures). §26 Raw-LLM /120 carried at **120**.)
**Updated-prev:** 2026-05-11 (Sess-79 B-27-§26 — created `00-tier1-bundle.md` v1.0.0 + walker-cost reflexivity lever in same turn (mirror-septet step 7/7 → **mirror-septet anchor CLOSED**); §26 R-band 119 → **120/120**.)
**Updated-prev:** 2026-05-11 (Sess-70 B-25 — Sess-58 A-47 deferred binding ratified in §97 as **AC-DG-24**. §26 Raw-LLM C5 19 → 20.)
...
> **Diagram parity gate (gate #41 / slot 63 — Sess-67 G-6x):** Diagram parity with §22 is mechanically enforced. Every `.mmd` file's `consumes:` binding is load-proven by `linter-scripts/check-diagram-parity.py`: each non-meta `.mmd` MUST appear in the §00 `consumes:` block (and vice versa); `01-er-diagram.mmd` MUST cover the §22 core entity set `{Profile, GitProfile, Repo, App, AppLink, AppLinkType, ShaRegistry, Pipeline}`; `09-endpoints-mindmap.mmd` MUST cover the App-write endpoint baseline `{append-log, fixed-log, clear-log}`; no `.mmd` may contain emoji codepoints in the lexer-fragile ranges (`U+1F300-1FAFF`, `U+2600-27BF`, `U+1F000-1F2FF`). Self-enforcing via §27 backlog gate `diagram-parity-check` (slot 63 doc) — wired in `.github/workflows/spec-health.yml` step "§26 diagram parity gate (#41 / G-6x / slot 63)" with `--self-test` 6/6 fixtures + live disk run, both hard-fail. AC-DG-01 (cardinality) + AC-DG-02 (coverage) promoted from contract-proven to load-proven.
