...
## Normative Contract (Phase 50) + Typed-Language Reference (Phase 54)

> **Promoted to detail file (Phase 153 Task S25-03):** the full Phase-50 normative contract block (CONTRACT/INV-*/FAIL-*/DEL-*), the inlined JSON Schema 2020-12 issue record, the TypeScript IssueStatus + IssueSeverity enums, and the Phase-54 Go/Rust/Python typed-language consumer references now live in [`./01-phase-50-54-contracts-detail.md`](./01-phase-50-54-contracts-detail.md). The split keeps the §25 audit-corpus bundle under the 140 KB walker cap so all 12 module files fit. Severity-enum disambiguation: `{blocker,major,minor,info}` in the detail file is **frozen historical** Phase-50 classifier; canonical issue-record severity is `{Critical,High,Medium,Low}` per parent `../97-acceptance-criteria.md` AC-AI-14.
