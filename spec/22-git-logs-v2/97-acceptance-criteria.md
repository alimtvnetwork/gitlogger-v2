...
- **Given** a client sends `/append-log` (endpoint #1 per §04) under `Transfer-Encoding: chunked` (no `Content-Length`), and the server MUST stream-parse + INCREMENTALLY enforce §10 caps + atomically roll back on any breach.
- **When** an AI auditor or implementer walks spec/22 §97 and seeks the binding contract for streaming ingest — stream-consume rule, incremental cap enforcement (`MaxPushPayloadBytes`/`MaxLinesPerPush`/`MaxLineBytes`), atomic rollback per AC-13, slow-loris timeout (`AppendLogMaxStreamSec` default 30s → `GL-INGEST-TIMEOUT`), buffered-fallback for `Content-Length`, and the 11-row T-INGEST-* scenario matrix.
- **Then** the canonical normative body MUST live in **§55 §AC-12 detail** (`55-ac-section-d-detail.md`) — stream-consume one JSON value / NDJSON line at a time; NO whole-body buffering (peak heap <64 MiB); incremental abort with `GL-PAYLOAD-TOO-LARGE` HTTP 413 + `Message` carries `"aborted at line N"`; full atomic rollback per AC-13 sticky-`HasError`; 30s wall-clock cap per `ConfigKv.AppendLogMaxStreamSec` → `GL-INGEST-TIMEOUT`; buffered ingest acceptable for `Content-Length` payloads. Plus the 11-row T-INGEST-01..T-INGEST-11 scenario matrix (chunked happy path · buffered happy path · 3 cap breaches · slow-loris · memory bound · atomic rollback · logger gating · clock-injection · per-SHA cursor isolation).
- **Verifies:** brief §Endpoints.2.b + §04 endpoint #1 + §10 cap matrix + §15 (`GL-PAYLOAD-TOO-LARGE` / `GL-INGEST-TIMEOUT` / `GL-VALIDATION-FAILED` / `GL-CURSOR-CONFLICT`) + AC-13 atomicity + AC-29 §5 clock injection. Closes Lesson #19 audit-boundary gap for streaming-ingest. Mirror of AC-05's T-DEDUP-* matrix on the second testable-scenario axis. **§55 §AC-12 detail** — full normative body lives in `55-ac-section-d-detail.md`.
- **Test invariant:** Mid-stream cap breach (line N exceeds `MaxLineBytes`/`MaxLinesPerPush`/`MaxPushPayloadBytes`) → HTTP 413 `GL-PAYLOAD-TOO-LARGE` with `Message` `"aborted at line N"` + atomic rollback of lines 1..N-1; peak heap <64 MiB proves no whole-body buffering.
- **Worked example:** POST `/append-log` chunked, 10 001 × 100 B → server reads stream, increments line counter, on line 10 001 returns 413 `{Code:"GL-PAYLOAD-TOO-LARGE", Message:"aborted at line 10001"}`; SELECT COUNT(*) FROM LogEntry WHERE PipelineId=X unchanged from pre-request.
- **Source:** `97-acceptance-criteria.md` (this AC) + `55-ac-section-d-detail.md` (full body); cross-refs `34-phpunit-test-skeleton.md`, `32-cli-test-plan.md`, AC-05, AC-13, AC-14, AC-29 §5, AC-80.
...
- **Test invariant (T-HE-01..T-HE-03):** After `/append-log {HasError:true}` for `PipelineId=P`, every subsequent read MUST report `Pipeline.HasError=1` until `/fixed-log {PipelineId:P}` succeeds; post-`/fixed-log`, the value MUST be `0`. A successful `/append-log` with `HasError=false` AFTER a sticky `HasError=true` MUST NOT clear the flag (only `/fixed-log` clears).
- **Worked example:** `POST /append-log {PipelineId:42, HasError:true}` → 200; `GET /get-pipeline-logs?PipelineId=42` → response shows `Pipeline.HasError=1`; `POST /append-log {PipelineId:42, HasError:false}` → 200; re-read → `HasError` STILL `1` (T-HE-02); `PUT /fixed-log {PipelineId:42}` → 200; re-read → `HasError=0` (T-HE-03).
- **Verifies:** brief §Endpoints.2.c, §04, AC-23 (CHECK constraint enforcement of 0/1 domain).
...
- **Test invariant (T-ACK-01..T-ACK-05):** Every write response (#1–#4) MUST conform to `AckResponse` exactly; `Retrieval.{PipelineLogs,PipelineErrorLogs,RepoLogs}` MUST start with `https?://` and contain `/wp-json/git-logs/v2/`; `RequestId` MUST equal the matching `AuditTrail.RequestId` row; `Retrieval` URLs MUST NOT contain `?token=` / `&token=` / `Authorization=` substrings; `/clear-log{,-all}` responses MUST include `Retrieval` (URLs return empty pages, NOT absent).
- **Worked example:** `POST /append-log {PipelineId:42, ...}` → response: `{"Status":"Ok", "RequestId":"7f3a-...", "WrittenCount":12, "Pipeline":{"PipelineId":42,"Branch":"main","Sha":"abc..."}, "Retrieval":{"PipelineLogs":"https://example.com/wp-json/git-logs/v2/get-pipeline-logs?PipelineId=42", "PipelineErrorLogs":"https://example.com/wp-json/git-logs/v2/get-pipeline-error-logs?PipelineId=42", "RepoLogs":"https://example.com/wp-json/git-logs/v2/get-logs?RepoUrl=...&Branch=main"}}` → assert `SELECT 1 FROM AuditTrail WHERE RequestId='7f3a-...'` returns 1 row (T-ACK-03).
...
- **Test invariant (T-ERR-01..T-ERR-06):** Error body keys MUST be exactly `{Status,Code,Message,RequestId,HttpStatus}` (no extras); `Code` ∈ `ErrorCode` enum (§17); `HttpStatus` MUST equal the response status code; `RequestId` MUST be server-generated UUIDv4 (client `X-Request-Id` header IGNORED); `RequestId` MUST equal the matching `AuditTrail.RequestId`; `Message` MUST NOT contain `/`, `\\`, `SELECT `, `Stack trace:`, or `at line ` substrings (path/SQL/stack-leak guard).
- **Worked example:** Send `GET /get-logs` with bad token + header `X-Request-Id: attacker-injected` → response 401 `{"Status":"Error","Code":"GL-AUTH-INVALID-TOKEN","Message":"Authentication failed","RequestId":"a0b1-server-uuid","HttpStatus":401}`; assert `array_keys($body) === ['Status','Code','Message','RequestId','HttpStatus']` (T-ERR-01); assert `$body['RequestId'] !== 'attacker-injected'` (T-ERR-04); assert `SELECT RequestId FROM AuditTrail WHERE RequestId='a0b1-server-uuid'` returns 1 (T-ERR-05).
...
- **Test invariant (T-MET-01..T-MET-03):** Authenticated `HistoryView` request MUST return `Content-Type: text/plain; version=0.0.4` (Prometheus exposition); body MUST parse against the Prometheus exposition grammar; missing permission MUST return HTTP 403 with `ErrorEnvelope` per AC-30; `wp_cron` flush MUST refresh counters at ≤5-minute cadence (assert `now() - lastFlushAt <= 300s`).
- **Worked example:** Auth as `HistoryView`-bearing user, `GET /wp-json/git-logs/v2/metrics` → 200, body starts with `# HELP gitlogs_append_log_total ...\n# TYPE gitlogs_append_log_total counter\ngitlogs_append_log_total 1234\n`; auth as user without `HistoryView` → 403 `{"Code":"GL-AUTH-FORBIDDEN", ...}` (T-MET-02).
- **Verifies:** §20, AC-30 (error envelope on 403).
...
- **Test invariant (T-OAS-01..T-OAS-03):** OpenAPI 3.1 parser load MUST succeed (zero errors); the resolved spec MUST contain exactly the 8 HTTP paths backing the 10 logical endpoints from AC-11 (`/append-log`, `/fixed-log`, `/clear-log`, `/clear-log-all`, `/get-logs`, `/get-pipeline-logs`, `/get-error-logs`, `/get-pipeline-error-logs`); `components.schemas.ErrorEnvelope` MUST mirror the shape enforced by AC-30 with every `ErrorCode` enum value documented in `15-error-codes.md` (no orphan codes in either direction).
- **Worked example:** CI step `npx @redocly/cli lint spec/22-git-logs-v2/17-openapi.yaml` → exit 0; `jq -r '.paths | keys[]' <parsed.json>` → 8 lines matching the AC-11 path set; `comm -3 <(jq -r '.components.schemas.ErrorCode.enum[]' parsed.json | sort) <(grep -oE 'GL-[A-Z-]+' spec/22-git-logs-v2/15-error-codes.md | sort -u)` MUST output zero lines.
- **Verifies:** §17, §15 (`GL-*` code catalog parity), AC-11 (10 logical / 8 HTTP endpoint inventory), AC-30 (error envelope shape).
...
- **Given** the plugin's logger subsystem (per §06) AND the `ConfigKv.LogLevelMin` row holding one of the six FIXED PascalCase strings (`Trace`/`Debug`/`Info`/`Warn`/`Error`/`Fatal`; default `Info`).
- **When** an AI auditor or implementer walks spec/22 §97 and seeks the binding contract for level gating — call-boundary gate, constant-time early return, forbidden sink-side work enumeration, "filter-at-sink" anti-pattern, cached-integer rule, request-scoped cache semantics, fixed-six-levels enum, `Fatal`-mode near-silence, and AC-05 dedup ordering, plus the T-LG-01..T-LG-09 scenario matrix.
- **Then** the canonical normative body MUST live in **§59 §AC-04 detail** (`59-ac-section-e-logger-detail.md`) — canonical integer mapping `Trace=0..Fatal=5` (lower = more verbose); gate at call boundary BEFORE any sink work; `if call_level < min_level` → `return` immediately (no `sprintf`, no `json_encode($ctx)`, no `debug_backtrace`, no timestamp, no source-location); cached integer read ONCE at request start (re-reading `ConfigKv` per call is a SPEC VIOLATION amplifying load 10–100×); cache is request-scoped (operator UI updates take effect NEXT request); six levels are FIXED (no `Verbose`/`Critical`/`Notice`); `LogLevelMin=Fatal` drops `Error` lines (near-silence emergency mode); AC-05 dedup applies AFTER gating. Plus the 9-row T-LG-01..T-LG-09 scenario matrix.
- **Test invariant:** With `LogLevelMin=Info`, 1000 `Logger::debug(...)` calls MUST produce zero sink invocations, zero `sprintf` calls, zero `json_encode` calls, zero `debug_backtrace` calls, and zero DB queries (cached integer holds across the loop).
- **Worked example:** Bootstrap caches `min=2`; hot loop `for ($i=0;$i<1000;$i++) Logger::debug("row $i", ['id'=>$i]);` → `assert($db->queryCount === $countBefore)` AND `assert($sink->callCount === 0)`. A "filter-at-sink" implementation would `sprintf` 1000× and allocate 1000 context arrays — measurable latency under high log volume.
- **Verifies:** brief §3.f, §06 (logger subsystem), §03 (`ConfigKv` admin), §23 (PascalCase enum storage), AC-05 (dedup post-gate ordering), AC-12 (ingestion logger respects same gate), AC-29 §5 (clock injection for tests). Closes Lesson #19 audit-boundary gap (pre-promotion AC-04 was a single dense paragraph with no test enumeration). Mirror of AC-05 (§53), AC-12 (§55), AC-23 (§56), AC-11 (§57), AC-34 (§58). **§59 §AC-04 detail** — full normative body lives in `59-ac-section-e-logger-detail.md`.
- **Source:** `97-acceptance-criteria.md` (this AC) + `59-ac-section-e-logger-detail.md` (full body); cross-refs `06-migrations-and-logger.md`, `03-admin-ui.md`, AC-05, AC-12, AC-29 §5, AC-80.

### AC-05 — Diagnostic dedup window  `[active]`

- **Given** the plugin's diagnostic-logger pipeline (per §06) processing a fingerprinted log stream where the dedup carve-out + 11-row T-DEDUP-* testable scenario matrix governs sink emission.
- **When** an AI auditor or implementer walks spec/22 §97 and seeks the binding contract for the dedup window — fingerprint composition, rolling-60s semantics, error/fatal carve-out, LRU bound, AC-04 ordering, per-PHP-process scope, and the 11-row PHPUnit/bats scenario matrix.
- **Then** the canonical normative body MUST live in **§53 §AC-05 detail** (`53-ac-section-e-detail.md`) — fingerprint = `(source, level, message_template, redacted_context_hash)`; rolling 60s window from FIRST occurrence; suppressed-count suffix `[deduped: N within 60s window starting <iso8601>]` on window-expiry next emit; NO suffix when count=0; `Error`/`Fatal` lines emit verbatim; state in process-local LRU (1024 fingerprints, NOT DB / Redis / file); MUST run AFTER AC-04 gate, BEFORE sink invocation; per-PHP-process (cross-worker dedup acceptable). Plus the 11-row T-DEDUP-01..T-DEDUP-11 scenario matrix + assertion contract (`test/AcDedupWindowTest.php` / `test/ac-dedup-window.bats` — T-ID in test method name; `GL-TEST-COVERAGE-GAP` on absence; deterministic monotonic-clock injection per AC-29 §5).
- **Verifies:** brief §3.g + §06 + AC-04 ordering + AC-12 ingest dedup symmetry. Closes Lesson #19 audit-boundary gap (pre-E1 AC-05 was a single dense paragraph with no test-case enumeration). Mirror of AC-23 / AC-11 promotions. **§53 §AC-05 detail** — full normative body lives in `53-ac-section-e-detail.md`. Test fixture skeletons in `34-phpunit-test-skeleton.md` (PHPUnit) and `32-cli-test-plan.md` (bats) per AC-80 sibling delegation; per Lesson #36 those files MUST cite this AC + T-ID by anchor, not restate scenario semantics.
- **Test invariant:** Identical fingerprint within 60s window → 1 emit + suffix `[deduped: N within 60s]` on expiry; `Error`/`Fatal` MUST emit verbatim every time.
- **Worked example:** 5× same `(ingest.streaming, Info, "saved profile %d", hash=abc)` at t=0,15,30,45,59s → 1 line at t=0; at t≥60s+ε → `"saved profile 42" [deduped: 4 within 60s window starting 2026-05-10T12:00:00Z]`.
- **Source:** `97-acceptance-criteria.md` (this AC) + `53-ac-section-e-detail.md` (full body); cross-refs `34-phpunit-test-skeleton.md`, `32-cli-test-plan.md`, AC-04, AC-12, AC-23, AC-29 §5, AC-80.
...
- **Given** any table or JSON payload in the codebase + the SQLite DDL in `18-schema.sql` declaring 5 normative CHECK constraints (4 boolean-domain + 1 exactly-one polymorphic).
- **When** an AI auditor or implementer walks spec/22 §97 and seeks the binding contract for identifier conventions + the canonical CHECK-constraint inventory + cross-language storage convention + tri-state exception + migration discipline + drift binding.
- **Then** the canonical normative body MUST live in **§56 §AC-23 detail** (`56-ac-section-d-constraints-detail.md`) — PascalCase for tables/columns/JSON keys/enum values; PKs are `INTEGER AUTOINCREMENT` named `{Table}Id`; 5-row CHECK catalog (`GitProfile.IsOrganization` L129 · `AppLink` exactly-one polymorphic L179-183 · `Pipeline.HasError` L198 · `Pipeline.PreviousHasError` L206 · `SshKey.IsActive` L316); cross-language storage delegated to spec/04 §2.1 (Go/PHP/Rust/C#/TS boundary scan rules — link-don't-restate); tri-state NULL exception clause routed to spec/04 §2.1; migration discipline bound to AC-29; same-PR three-surface invariant (DDL + this catalog + §98 row) with `GL-SCHEMA-DRIFT` on partial landing.
- **Verifies:** brief §DB.2–4 + §02 + §18; closes Lesson #19 audit-boundary gap for SQLite domain enforcement. Mirror of AC-11 (endpoints) and AC-81 (enums) — three Phase D tier-1 normative-surface promotions. **§56 §AC-23 detail** — full normative body lives in `56-ac-section-d-constraints-detail.md`.
- **Test invariant:** Every `IN (0,1)` boolean column + the `AppLink` exactly-one polymorphic CHECK MUST appear in `18-schema.sql` AND this catalog AND a §98 changelog row in the same PR; partial landing = `GL-SCHEMA-DRIFT` and CI-blocking.
- **Worked example:** `grep -nE "CHECK \(" spec/22-git-logs-v2/18-schema.sql` → exactly 5 hits (lines 129, 179, 198, 206, 316); a 6th hit without a matching new row in this catalog fails the `check-spec-folder-refs.py` linter.
- **Source:** `97-acceptance-criteria.md` (this AC) + `56-ac-section-d-constraints-detail.md` (full body); cross-refs `18-schema.sql`, spec/04 §2.1, spec/23 AC-ADB-14, AC-13/18/29/55/60/64/77, AC-78/79/81.
...
- **Given** a WordPress multisite installation (per §24) — both flavors apply: network-activated AND per-site activated; N subsites each with its own `blog_id` and `wp_<blog_id>_*` table prefix.
- **When** an AI auditor or implementer walks spec/22 §97 and seeks the binding contract for multisite tenancy — per-site DB path resolution, single-shared forbidden, site-scoped reads, no aggregated UI, blog-switch connection invalidation, identical schema, lazy migration, deletion cleanup, single-site fallback, and the T-MS-01..T-MS-09 scenario matrix.
- **Then** the canonical normative body MUST live in **§58 §AC-34 detail** (`58-ac-section-e-multisite-detail.md`) — per-site SQLite DB resolved via `wp_upload_dir()` (subsite #N → `/wp-content/uploads/sites/N/git-logs/plugin.db`); single shared DB across subsites FORBIDDEN; reads strictly site-scoped (no cross-subsite fan-out); NO network-admin aggregated view; `switch_to_blog($id)` MUST invalidate the DB connection (cache key = `(blog_id, db_path)`); `restore_current_blog()` MUST symmetrically restore; migration runner runs **lazily per-subsite** on first access (eager init FORBIDDEN); `wp_delete_site($id)` MUST remove `sites/<id>/git-logs/`; single-site install treated as `blog_id=1` with NO `is_multisite()` branch in the DB locator. Plus the 9-row T-MS-01..T-MS-09 scenario matrix.
- **Test invariant:** `switch_to_blog(N)` followed by next DB access MUST open a NEW connection against `sites/N/git-logs/plugin.db`; reusing the prior `blog_id=1` connection across the boundary is a tenancy bug (silent cross-tenant write).
- **Worked example:** Network-admin loop visits subsites: `$c1 = $registry->current(); switch_to_blog(3); $c3 = $registry->current();` — assert `$c3 !== $c1` and `$c3->path === '/wp-content/uploads/sites/3/git-logs/plugin.db'`; `restore_current_blog()` then `$registry->current() === $c1`.
- **Verifies:** §24 (multisite per-site DB contract), AC-03 / AC-29 (migration runner per-site lazy), AC-21 (audit tables per-site), AC-12 (ingest writes to active subsite). Closes Lesson #19 audit-boundary gap (pre-promotion AC-34 was a 9-clause dense paragraph with no test enumeration). Mirror of AC-05 (§53), AC-12 (§55), AC-23 (§56), AC-11 (§57). **§58 §AC-34 detail** — full normative body lives in `58-ac-section-e-multisite-detail.md`.
- **Source:** `97-acceptance-criteria.md` (this AC) + `58-ac-section-e-multisite-detail.md` (full body); cross-refs `24-multisite.md`, AC-03, AC-12, AC-21, AC-29, AC-80.
...
### AC-22-CE1 — Co-edit cohorts: schema/endpoint/CLI changes MUST move sibling files in the same commit  `[active]`

- **Given** the §22 module's "AI Implementer Quickstart" (`00-overview.md` lines 64+) declares three normative co-edit cohorts that prevent silent drift between contract surfaces:
  - **Schema cohort:** `02-database-schema.md` + `18-schema.sql` + `51-ac-enum-catalog-detail.md` + sibling §26 `01-er-diagram.mmd` (+ `.svg` re-render)
  - **Endpoint cohort:** `04-rest-api-endpoints.md` + `17-openapi.yaml` + `15-error-codes.md` (+ `57-ac-section-d-endpoints-detail.md` if the AC body shifts)
  - **CLI cohort:** `40-cli-overview.md` + the changed protocol file under `41-44` + `54-ac-j-series-cli-detail.md`
- **When** any commit (human or AI) touches **one** file from a cohort,
- **Then** the same commit MUST also update **every other file in that cohort that the change observably affects** — a schema-cohort change to a column type MUST update the DDL, the prose schema, the enum catalog (if the column is enum-typed), and re-render the §26 ER diagram + companion `.svg`; an endpoint-cohort change to a request shape MUST update the markdown contract, the OpenAPI schema, and the error-codes table (if new error codes surface); a CLI-cohort change to a wire frame MUST update the CLI overview, the affected protocol file, and the J-series AC detail. **Exemptions:** typo fixes, comment-only edits, and version-bump-only commits are exempt provided the diff contains no semantic surface-area change. **Failure mode:** a cohort commit that omits a sibling file produces a "split-brain" spec where the contract files disagree — readers MUST treat such a state as a defect and the next commit MUST close the cohort.
- **Verification (machine-checkable):** a CI lint over `git diff --name-only HEAD~1` MAY assert: if `02-database-schema.md` ∈ diff AND `18-schema.sql` ∉ diff → fail with `AC-22-CE1-SCHEMA-COHORT-INCOMPLETE`; same pattern for endpoint and CLI cohorts. Until the lint ships, AC-22-CE1 is enforced by reviewer attestation in the PR description ("Cohort: schema · files updated: 02, 18, 51, §26/01 — N/A: enum-catalog (no enum touched)").
- **Verifies:** §00 `00-overview.md` "AI Implementer Quickstart" hard-rules block (Session 9 — promotes the advisory cohort rules to an enforceable contract); closes the F-10 advisory-vs-enforceable gap surfaced in the Session 10 scorecard. Mirror of the §26 AC-DG-23 pattern (Session 11) that lifted `.mmd` header rules from advisory to enforceable.
...
- **Given** spec/22's full on-disk asset inventory comprises **37 entries** (§99 inventory authoritative): root `.md` files `00-overview.md` through `08-*`, `14-*` through `37-*` (locked-vacant slots `09-13` enforced absent per AC-22-LV1), `39-split-db-log-storage.md`, `97`/`98`/`99`, plus normative non-`.md` fixtures `18-schema.sql` (canonical DDL, 465 lines), `33-bats-test-skeleton`, `34-phpunit-test-skeleton` (311 lines, PHP class skeletons), `35-reference-ci-yml`. The audit harness (spec/27 slot 34 v1.1.0 AC-34-09) bundles only `{00,97,98,99}-*.md` at a tier-1 cap of ~90 KB and truncates non-tier-1 siblings at the per-bundle 140 KB walker cap.
- **When** an AI auditor walks the tier-1 bundle and emits any of the four canonical false-positive findings — `[D5] Missing Core Schema and API Definitions` (citing files 04, 18, 34, or "36 files in the inventory"), `[D4] Truncated Glossary and Enums` (citing `01-glossary-and-enums.md` "truncated at the 136 KB cap"), `[D4] Missing Concrete Code Fixtures` (citing missing SQL DDL or PHP class skeletons), or `[D3] Concurrency/Race Condition on Rate Limits` (citing AC-26's "in-memory + persisted floor" without spec/13 AC-22 binding) — OR reports "missing files 09-13" against the AC-22-LV1 locked-vacant range.
- **Then** the auditor MUST classify each as a **harness bundling-cap artifact** (NOT a spec defect) and the contributor MUST verify by running the canonical Lesson #39 evidence triple before opening any §99 row: `wc -l <file>` + `tail -20 <file>` + `grep -c '^### ' <file>`. Verification anchors (byte-exact, MUST hold across phases): `04-rest-api-endpoints.md` = 406 lines on disk, `18-schema.sql` = 465 lines (canonical DDL), `34-phpunit-test-skeleton.md` = 311 lines (PHP skeletons), `01-glossary-and-enums.md` = 14 346 bytes (14 KB — well under any reasonable cap; truncation occurs during multi-file bundle assembly when sibling files exhaust the byte budget BEFORE `01-*.md` is appended; `tail -20` shows the closing Drift-detection contract table = full TypeScript Mirror + Enum Catalog present). The persisted-floor concurrency finding is bound to **spec/13 §97 AC-22** per Lesson #36 (link-don't-restate) — the SQLite WAL/busy-timeout/retry contract lives in spec/13 canonical; restating in §22 = dual-source drift class. Locked-vacant slots `09-13` are enforced by AC-22-LV1 — auditor "missing files 09-13" reports MUST be treated as the LOCKED state, not a spec gap.
- **Test invariant:** every spec/22 phase MUST verify the 4-tuple `(04=406 lines, 18=465 lines, 34=311 lines, 01=14 346 bytes)` matches on-disk reality; any drift = inventory gap requiring §99 row + AC-78 evidence ledger update in the same PR. Conversely, any tier-1 audit finding citing one of the four canonical false-positive families above without including the Lesson #39 evidence triple in its own evidence cell = stale-cache artifact per Lesson #34 (auto-classifiable; deferred §27 gate `ac-78-false-positive-detector` will mechanically reject such findings).
- **Worked example:** v3/v4/v5 LLM-gateway audits (cached pre-A8) reported `[D5] Missing Core Schema` citing "file 18 absent". On-disk evidence: `wc -l spec/22-git-logs-v2/18-schema.sql` → 465; `tail -3 18-schema.sql` → trailing `COMMIT;` line. AC-78 classification = harness-cap artifact; §99 row NOT opened; finding logged in AC-78 evidence ledger as stale-cache instance #1 (precedent cited by AC-79/AC-80/AC-81 mirror-quartet members).
- **5-link self-enforcement chain** (mirror of A-44 AC-87 pattern): (1) Inventory pin (37-entry list above) → (2) per-file byte-exact verification anchor (4-tuple) → (3) Lesson #39 evidence triple command (`wc -l` + `tail` + `grep`) → (4) cross-module link to spec/13 AC-22 (concurrency boundary, restate-forbidden) → (5) deferred §27 gate `ac-78-false-positive-detector` (mechanical executor). Breaking any link = AC-78 breach; gate #5 will load-prove the chain end-to-end once shipped.
- **Invalidation triggers** (any = AC-78 breach + new §99 row in same PR): (a) on-disk file count ≠ 37; (b) any anchor in the 4-tuple drifts (e.g., 04 ≠ 406 lines after a phase edit without anchor refresh); (c) any new sibling file added without an AC-80 delegation-map row in the same PR; (d) AC-22-LV1 locked-vacant slot `09`-`13` becomes occupied; (e) spec/13 AC-22 SQLite WAL contract restated inline anywhere in spec/22.

- **Verifies:** the spec/22 module-kind = `module` declaration AND the auditor-authoritative inventory contract for the 37-entry asset list (including normative non-`.md` fixtures `18-schema.sql` + `33`/`34`/`35` test skeletons) AND the locked-vacant-slot AC-22-LV1 contract AND the cross-module link-not-restate boundary for AC-26's persisted-floor concurrency; codifies **Lesson #29** + **Lesson #36** for tier-1-bounded auditors. Mirror of spec/13 AC-24 + spec/28 AC-28-41 + spec/14 AC-21 + spec/16 AC-21 + spec/07 AC-35 + spec/10 AC-9 + spec/03 AC-08 + spec/11 AC-10 + spec/12 AC-09 + spec/17 AC-10 + spec/18 AC-09 + spec/25 AC-AI-09..11. Now structured per A-44 5-link self-enforcement chain (Sess-59 A-48): inventory pin → byte-exact anchors → Lesson #39 evidence triple → cross-module link → deferred §27 mechanical gate. Until A8 (LLM-gateway re-score) unblocks, the cache will report v3/v4/v5 [D5]/[D4]/[D3] findings citing these — this AC declares those findings stale-cache artifacts per Lesson #34.
...
- **Test invariant:** Every external citation in spec/22 (spec/04 DDL · spec/12 CI · spec/13 AC-22 SQLite · spec/26 Mermaid · spec/05 split-DB · spec/27 linter) MUST appear as exactly one row in the §50 6-row table with `Restate-in-22 forbidden? = YES`; restating any cited contract inline = Lesson #36 dual-source drift.
- **Worked example:** spec/22 §39 cites `spec/13 §97 AC-22` for SQLite WAL/busy-timeout/retry → §50 row exists `(spec/13 AC-22, 39-split-db-log-storage.md, SQLite WAL semantics, YES)`; §39 body says "per spec/13 AC-22" not "WAL with `busy_timeout=5000`...".
...
- **Test invariant:** Every spec/22 sibling slot 01-96 holding a tier-1 normative contract MUST appear as a row in the §50 33+ row delegation map (with owning §97 AC + restate-forbidden flag); a sibling without a row = unbound contract surface = Lesson #21 audit-boundary gap.
- **Worked example:** `46-server-upload-frames-endpoint.md` ships → AC-80 gains row 39 `(46, AC-88, K1 receiver, YES)` in the same PR; a row 39 with no §46 file or §46 with no row 39 = CI-blocking.
...
- **Test invariant:** `riseup-git-logs` binary MUST expose exactly the 6 closed-set subcommands (`init`/`pre-push`/`post-push`/`doctor`/`status`/`autofix`) with stable `ExitCode` enum per §40; new subcommand without an AC-82 row = unbound surface.
- **Worked example:** `riseup-git-logs --help` lists 6 subcommands; `riseup-git-logs frob` → `ExitCode.UNKNOWN_SUBCOMMAND` (= 64) and stderr `GL-CLI-UNKNOWN-SUBCOMMAND`; a 7th subcommand requires a §40 row + §54 detail update + §97 AC-82 stub bump in same PR.
...
- **Test invariant:** `.riseup-git-logs.json` MUST contain exactly the 8 fields per §41 §3 (`schemaVersion`/`repoIdentityHash`/`uploadUrl`/`expiresAt`/`autofixDownloadUrl`/`lastUploadedHead`/`lastUploadAt`/`pointerSignature`); top-level field outside this set = `GL-PTR-FORBIDDEN-FIELD`.
- **Worked example:** Server response from K3 `/refresh-pointer` → CLI writes `.riseup-git-logs.json` with the 8 fields in canonical lex order; `jq 'keys'` returns `["autofixDownloadUrl","expiresAt","lastUploadAt","lastUploadedHead","pointerSignature","repoIdentityHash","schemaVersion","uploadUrl"]`; a 9th key = `GL-PTR-FORBIDDEN-FIELD` and CLI rejects.
...
- **Test invariant:** Each candidate frame produced by `pre-push` MUST be classified into exactly one of the §42 closed-set outcomes (`OK`/`WARN`/`ERROR`/`INTERNAL`/`SKIP`) per the deterministic rule table; a frame escaping the 5-state set = `GL-CLASSIFY-DRIFT`.
- **Worked example:** Frame `{exitCode:1, stderr:"fatal: ..."}` → classifier returns `outcome=ERROR` per §42 row 3; `{exitCode:0, stderr:"warning: deprecated"}` → `WARN`; classifier MUST NOT emit `outcome=null` or unknown values (CI test asserts JSON-schema enum on every frame).
...
- **Test invariant:** `riseup-git-logs` POSTs NDJSON-over-HTTPS to server `/upload-frames` under SSH-key Lane B with REQUIRED `Idempotency-Key` UUIDv4 + retry on `GL-UPLOAD-RETRY-*` only (NOT on `GL-AUTH-*`); response MUST be the 6-field §43 §6 envelope.
- **Worked example:** `pre-push` upload → `POST /wp-json/git-logs/v3/upload-frames` `Content-Type: application/x-ndjson` `Idempotency-Key: 7f3a...` body 50 frames; 200 → `{batchId, framesReceived:50, framesDeduplicated:0, uploadedAt, expiresAt, autofixDownloadUrl:null}`; replay with same `Idempotency-Key` returns identical body (cached).
...
- **Test invariant:** Closed 5-suite test matrix `T-CLI-{IDENT,PTR,CLASSIFY,UPLOAD,AUTOFIX}` with Min-cases floors (7/9/11/13/12); decreasing any floor or adding a 6th tier-1 suite = `GL-TEST-PLAN-DRIFT` and CI-blocking.
- **Worked example:** `go test ./pkg/identity/...` → ≥7 cases bearing `T-CLI-IDENT-*`; `bats test/cli-upload.bats` → ≥13 cases bearing `T-CLI-UPLOAD-*`; coverage report shows L1 ≥85% line on `pkg/{identity,pointer,classifier,upload,autofix}`; absent T-ID = `GL-TEST-COVERAGE-GAP`.
...
- **Test invariant:** Server `POST /wp-json/git-logs/v3/upload-frames` MUST run the 10-step strict validation order with auth (step 4) BEFORE body parse (steps 5-8); reordering = security regression. Lane A → 401 `GL-AUTH-LANE-MISMATCH`; v2 path coalescing FORBIDDEN.
- **Worked example:** Lane A `POST /upload-frames` with valid NDJSON body → server returns 401 `GL-AUTH-LANE-MISMATCH` WITHOUT parsing the body (proven by zero rows in `LogEntry` + auth check in access log preceding any parse log line).
...
- **Test invariant:** Server `GET /autofix/{repoIdentityHash}/{frameId}` is single-consumption (`consumedAt` set atomically per spec/13 AC-22); 200 once, 410 `GL-FIX-CONSUMED` thereafter; expired bundle = 410 `GL-FIX-EXPIRED` BEFORE consumption check (more actionable). `If-None-Match` 304 does NOT count as consumption.
- **Worked example:** First GET → 200 + bundle body + `ETag: "sha256-..."` + `Cache-Control: private, no-store`; same client repeats → 410 `{Code:"GL-FIX-CONSUMED"}`; another client with stale `If-None-Match` matching ETag → 304 (no body, no consumption tick).
...
- **Test invariant:** Server `POST /refresh-pointer` is the ONLY endpoint that mints a fresh 8-field §41 pointer payload WITHOUT uploading frames; the 10-entry forbidden body-top-level field list (frame-smuggle guard) is enforced before any DB write; auto-minting a `FixBundle` row FORBIDDEN.
- **Worked example:** `riseup-git-logs doctor` after partial init → POST `/refresh-pointer` `{repoIdentityHash, headSha, branchName, schemaVersion}` → 200 with the 8-field payload; same POST with extra top-level `frames:[...]` field → 400 `GL-PTR-FRAME-SMUGGLE` and ZERO new rows in `FixBundle`.
