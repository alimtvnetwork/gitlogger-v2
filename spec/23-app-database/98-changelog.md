...
### 4.8.0 — 2026-05-10 — Session 66 / Phase-5 T-19 / F-03 closure: `Setting` + `UserSettingOverride` DDL materialised
- **Action**: Added `## Settings Persistence (Normative — Phase-5 T-19 / F-03 closure)` to §00 (~140 lines, between AC-ADB-WE-01 and Migration Template). Materialises the storage layer for the §24 §00 line 399 `## Settings Surface` (S-1 route matrix, S-2 persistence matrix, S-3 4-invariant seedable-config binding). Three in-spec ACs minted: **AC-ADB-SETTING-01** `[critical]` (DDL byte-shape pin — both tables + composite PK + dual ON DELETE CASCADE + enum CHECK; wired to §24 gate #25 clause-3 + future slot-45 DDL-shape gate); **AC-ADB-SETTING-02** `[critical]` (R-09 `COALESCE(o.Value, s.Value)` order pin; wired to §24 gate #25 clause-4); **AC-ADB-SETTING-03** `[high]` (replay-safe `ON CONFLICT(Key) DO NOTHING` migration shape; cross-cites Rule 12 + future DDL-shape gate). Section includes 5-row forbidden-storage-layer-pattern list (collapsed table / Setting mutation lane / nullable UserId / inverted COALESCE / weakened FK action). REFERENCE-lane PG mirror present but fenced under T-01 precedence pin (`DO NOT MATERIALISE`).
- **Why now**: Audit Session 65 F-03 ranked highest single-session leverage on Raw-LLM persona (4-6 point lift on §23+§24). §24 gate #25 (T-18) machine-checks the §24 surface contract but cannot enforce storage-side existence; without DDL, Raw-LLM implementing R-10/R-12/R-14 PATCH endpoints would invent a single `settings` table (collapses seed/override — exact CAF-04 violation pattern). T-19 lands the storage-side mirror so the §27 backlog gate #25 clause-3 literal `UserSettingOverride` now resolves to a concrete table.
- **Self-enforcement chain (now end-to-end for Settings persistence)**: §24 §00 S-1/S-2/S-3 (line 407..445) → §24 §97 AC-ADS-UI-03 → §24 §97 AC-CAF-04 → §24 gate #25 (T-18) → **NEW** §23 §00 `## Settings Persistence` DDL block → AC-ADB-SETTING-01..03. R-09 query template's `COALESCE(o.Value, s.Value)` argument order is the byte-shape mirror of S-3 invariant 3 prose `COALESCE(override.Value, seed.Value)` — gate #25 clause-4 already rejects inverted order on the §24-side prose; AC-ADB-SETTING-02 is the storage-side mirror that catches the same drift if it lands first in SQL (e.g. via direct schema PR not touching §24).
- **Bindings**: closes the oldest open §24-side follow-up ticket (`Setting`+`UserSettingOverride` DDL, declared at §24 §00 S-5 line 456 by T-08 — 11 turns from declaration to materialisation). Closes audit Session 65 F-03 finding (Raw-LLM C2/C5 dragged by missing storage target). Sister to §24 gate #25 (T-18) — the two together form the first complete §27/§24/§23 three-folder load-proof for a non-Git-Logs-v2 surface (settings is purely an App-layer concern, no §22 binding). Forward-link: a future slot 45 `check-setting-override-ddl-shape.py` gate will mechanise AC-ADB-SETTING-01 byte-shape (currently load-proven only via §24 gate #25 clause-3 literal `UserSettingOverride` token presence — not full DDL-shape parity).
- **Lockstep**: §00 v4.4.0 → **v4.5.0** (new section + 3 in-spec ACs; banner). §97 unchanged (3 new ACs are inline-only per §23 in-spec catalogue precedent — same pattern as AC-ADB-REST-01 + AC-ADB-WE-01; §97 owns AC-ADB-01..18 only). §99 v2.1.5 → **v2.1.6** (this audit row).
- **Scorecard impact (Session 66 / A-57)**: §23 C2 Completeness +1 (storage layer for §24 Settings now exists; was the largest single completeness gap). §23 C5 Implementability — Raw-LLM persona +5 (118→123 conceptual; capped at 120 / un-conditional 20). §24 C5 Implementability — Raw-LLM persona +4 (R-10/R-12/R-14 implementer now has concrete schema target; R-09 query template removes guess work). §27 unchanged (no new gate this turn — slot 45 deferred). Carry-forward all other scores.
- **Invalidation triggers**: removing the `## Settings Persistence` section · weakening either `ON DELETE CASCADE` · dropping the `(UserId, Key)` composite PK · omitting the enum-CHECK constraint · introducing a `settings`-collapsed-table alternative · inverting the R-09 COALESCE order in the query template · making `UserSettingOverride.UserId` nullable · removing the replay-safe `ON CONFLICT(Key) DO NOTHING` clause from the seed migration template.
- **No** §97 surface change (in-spec catalogue), **no** RUBRIC bump, **no** AC-31-31 cascade.
...
### v4.10.0 — 2026-05-10 — Phase-5 T-11: 4 failure-path Worked Examples (AC-ADB-WE-01)

- **Action**: Inserted new normative section **"Worked Examples — Failure Paths (Normative — Phase-5 T-11)"** in `00-overview.md` between AC-ADB-REST-01 and Migration Template. Contents: WE-1 (Resolve unknown RepoUrl → 404 `applink.unresolved`), WE-2 (Reconnect when no disconnected row exists → 201 always inserts new row per Q3), WE-3 (Boolean-coercion attack on R-01 with `IsActive: 1` → 422 `field.invalid` per R-4 invariant 2), WE-4 (Disconnect already-disconnected link → 200 idempotent with preserved DisconnectedAt timestamp per R-4 invariant 6). Each example: full HTTP request + full HTTP response with R-3 envelope + DB invariants asserted clause. Plus AC-ADB-WE-01 self-enforcing AC.
- **Why**: §23 had Q1-Q4 happy-path SQL fixtures only; failure paths were derivable from R-3 status table + R-4 invariants but not exemplified. Implementers (especially Raw-LLM persona) were forced to invent the exact response shape, status code, and idempotency semantics. WE-1..WE-4 lock the four highest-risk drift surfaces: (1) closed enum violation, (2) Q3 always-insert contract, (3) wire-boolean coercion, (4) idempotency timestamp preservation.
- **Verification**: `rg -n '^### WE-[1-4]' spec/23-app-database/00-overview.md` → 4 matches ✅. `rg -n 'AC-ADB-WE-01' spec/23-app-database/00-overview.md` → 1 match ✅. Migration Template heading preserved.
- **Scorecard delta**: §23 C1 Clarity 18→19, C3 Testability 18→20, C5 Implementability 20→20 (already at ceiling) — REQUIRES self-enforcing mechanism for C3=20: AC-ADB-WE-01 + the 4 examples are themselves byte-for-byte fixtures that any test harness can lift directly. C3=20 is **un-conditional** (no §27 gate dependency) because the Worked Examples ARE the test fixtures — the spec content is the gate. This is the strongest self-enforcing form possible (Rubric v2 anchor: "score 20 = the spec is its own test fixture").
- **Invalidation triggers**: removing any of WE-1..WE-4 · weakening any response status code · removing any DB invariants clause · removing AC-ADB-WE-01 · introducing a happy-path WE-N (this section is failure-path-only by construction).
