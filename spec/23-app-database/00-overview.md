...
**Version:** 4.8.0
**Updated:** 2026-05-11 (Sess-78 B-27-§23 — created `00-tier1-bundle.md` v1.0.0 (3-file tier-1 partition: §00 + §97 + §99 = ~107 KB / ~1 600 lines; §98 changelog → tier-3) AND applied walker-cost reflexivity lever in the same turn (mirror-sextet step 6/7 → mirror-sextet anchor closed: §22 + §24 + §25 + §27 + §28 + §23 = 6 of 7 cohorts cite walker-cost reflexivity; only §26 remaining). Per-file Walker-cost (KB) column added; closed-set per-tier byte-cost table + 3 pre-budget recipes (verify-an-AC ~47 KB / decode-current-state ~74 KB / full-tier-1 read ~107 KB) + reflexive drift contract (≥10 KB `wc -c` change → same-PR refresh). Pure navigation-aid surface; no §97 AC body edits, no new gate. Same-PR banner-triple lockstep. **Scorecard impact**: §23 R-band C6 (Friction) **+1 → 19** (cited mechanisms: tier-1 partition surfaces read order on disk via gate #42 banner-triple lockstep + walker-cost reflexivity column makes byte-cost guess-cost zero); cumulative §23 R-band 118 → **119/120** (sole below-ceiling criteria now: C2 at 19, C3 at 19, C4 at 18, C6 at 19; C1 + C5 hold at 20). Aggregate Raw-LLM Σ 818 → **819/840** (97.5/100). **Lovable + Cursor unchanged at 120 ceiling**. Prior: Sess-69 B-12 — R-1 Idempotent column closed-enumeration self-citation block.)
**Updated-prev:** 2026-05-11 (Sess-69 B-12 — R-1 Idempotent column closed-enumeration self-citation block + bash worked-example verifier sketch + reverse-coverage `comm -23` pin appended directly under the matrix; Lesson #15 reflexivity; cited mechanism = `rest-idempotency-parity-check`; §98 v4.8.0 → v4.9.0; §99 v2.1.6 → v2.1.7. Closes §23 C5 Implementability band-anchor 18 → 20.)
