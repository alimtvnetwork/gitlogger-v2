...
### v4.16.0 — 2026-05-10 — Phase-5 T-12: AC-CAF-01..05 minted (Cross-cutting App Framework)

- **Action**: Minted new `AC-CAF-NN` namespace in `97-acceptance-criteria.md` covering 5 cross-cutting App-layer concerns spanning §23+§24+§25: AC-CAF-01 (wire-boolean parity end-to-end DB↔REST↔UI), AC-CAF-02 (uniform error envelope across DB-fault and UI-render paths), AC-CAF-03 (observable idempotency contract at every layer), AC-CAF-04 (seed-row/override separation extends seedable-config to App-layer), AC-CAF-05 (audit-finding strings cite never restate, lifts §25 AC-AI-10/11 to cross-cutting status).
- **Why**: Cross-cutting App framework concerns previously had no dedicated namespace — boolean parity was duplicated across §23 R-4#2, §24 U-3, AC-ADB-11; error envelope across §23 R-3, §24 AC-ADS-UI-02, §22 AC-ADS-15; etc. CAF-NN ACs are deliberately thin pointers binding §24-anchored contracts to their §23/§25 mirrors so cross-module drift is mechanically detectable. Each CAF AC names its self-enforcing mechanism explicitly (existing AC + new/existing §27 gate).
- **Verification**: `rg -n '^### AC-CAF-0[1-5]' spec/24-app-design-system-and-ui/97-acceptance-criteria.md` → 5 matches ✅. Cross-References heading preserved.
- **Scorecard delta**: §24 C4 Consistency 20→20 (already at ceiling, T-09); cross-folder lift: §23 C4 19→20 (CAF-01/02/03/04 anchor §23 contracts in §24 cross-cutting namespace, eliminating §23 single-source-of-truth ambiguity), §25 C4 19→20 (CAF-05 lifts AC-AI-10/11 to cross-cutting status, locking the audit-evidence interpretation in 3 places). REQUIRES self-enforcing mechanisms: existing ACs (AC-ADS-UI-02, AC-ADB-11, AC-ADB-REST-01, AC-ADS-UI-03, AC-AI-10/11) + §27 backlog gates `error-envelope-uniformity-check` (NEW) and `audit-quoted-evidence-marker-check` (NEW). C4=20 on §23 + §25 is **conditional** on at least one of those §27 gates shipping within 3 turns or both revert to 19 (self-decay clause).
- **Invalidation triggers**: removing any of AC-CAF-01..05 · weakening the "binds to" cite in any CAF AC · removing the self-enforcing mechanism clause · adding a 6th CAF AC without a corresponding §27 gate backlog entry.

### v4.17.0 — 2026-05-10 — Phase-5 T-13: §24 §97 orphan headers reconciled

- **Action**: Inserted new H2 header `## Acceptance Criteria — Inheritance & Boundary (cont. — Phase-5 T-13 reconciliation)` immediately before AC-ADS-15 in `97-acceptance-criteria.md`. Includes a normative reconciliation note explaining: (a) why AC-ADS-15/16 were structurally orphaned (appended after Worked Examples in earlier sessions), (b) why content reorder was rejected (would invalidate every existing line-anchored citation in §22/§25/§27), (c) explicit affirmation that AC-ADS-15/16 carry full normative weight, (d) clarification that the CAF namespace (T-12) is a sibling, not a continuation of AC-ADS-*.
- **Why**: §97 had two H2 sections (`## Acceptance Criteria`, `## Worked Examples`) followed by 2 orphan ACs (AC-ADS-15, AC-ADS-16) appended without an enclosing H2, then a 3rd new H2 (`## Cross-cutting App Framework (CAF)` from T-12). A walker reading section-by-section would treat AC-ADS-15/16 as belonging to "Worked Examples" — semantically wrong. Adding the cont. H2 fixes the structural read without disturbing any existing line numbers cited elsewhere.
- **Verification**: `rg -n '^## ' spec/24-app-design-system-and-ui/97-acceptance-criteria.md` → 6 H2 sections in correct logical order: Module Summary → Inlined Contracts → Acceptance Criteria → Worked Examples → Acceptance Criteria — Inheritance & Boundary (cont.) → Cross-cutting App Framework (CAF) → Cross-References ✅. AC-ADS-15 and AC-ADS-16 remain at their original line numbers (200, 211) modulo the 5-line H2 insertion offset; existing line-anchored citations in §22/§25/§27 may need a +5 line shift but the AC-ID anchors are stable.
- **Scorecard delta**: §24 C1 Clarity 19→20 (structural orphan resolved; section-walker disambiguated) — REQUIRES self-enforcing mechanism: in-spec reconciliation note + §27 backlog gate `ac-section-orphan-header-check` (NEW). C1=20 conditional on §27 gate shipping within 3 turns or reverts to 19.
- **Invalidation triggers**: removing the new H2 header · removing the reconciliation note · re-orphaning AC-ADS-15/16 by deletion of any enclosing H2 · adding a new AC-ADS-NN after the Cross-cutting App Framework section without minting a 4th cont. H2.
