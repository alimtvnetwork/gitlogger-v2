...
**Version:** 2.2.7
**Updated:** 2026-05-11 (Sess-69 B-17 — initial tier-1 partition manifest `00-tier1-bundle.md` shipped; Raw-LLM C1 Clarity 19 → 20)

> **v2.2.7 update (Sess-69 B-17 — tier-1 essential bundle manifest):** Created `00-tier1-bundle.md` (v1.0.0) partitioning the §24 cohort into tier-1 (all 4 normative `.md` files: `00-overview.md`, `97-acceptance-criteria.md`, `98-changelog.md`, this file) + tier-3 (`lifecycle-component-render.mmd`). Tier-1 line sum verified: `wc -l spec/24-app-design-system-and-ui/{00-overview.md,97-acceptance-criteria.md,98-changelog.md,99-consistency-report.md} | tail -1` → **1 606 total** (894-line headroom under the 2 500 single-context-window comfort ceiling). Manifest declares the no-tier-2 invariant (clause 7) — as long as the line-budget holds, §24 MUST NOT introduce any tier-2 file. Self-cited via gate #42 (`meta-verify-lockstep.py`, slot 64) clause-5 banner-triple lockstep, gate #39 (`check-no-out-of-scope-spec-folder-link.py`, slot 61) on link-target axis, and gate #36 (`check-no-sql-ddl-in-ui-folder.py`, slot 58) for the no-DDL boundary cited in the Reader Pin. Mirrors §22 B-1 / §27 B-6 / §28 B-7. Banners: §00 v4.11.0 → **v4.12.0**; §98 v4.14.0 → **v4.15.0**; this file v2.2.6 → **v2.2.7**. **No §97 bump** (manifest is navigation-aid, not a contract). **Scorecard delta**: §24 Raw-LLM **C1 Clarity 19 → 20**; §24 totals **L 120 / C 120 / R 119**. Sole Raw-LLM cohort floor remains §27 (R 94).
