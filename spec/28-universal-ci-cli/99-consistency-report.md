...
> **v2.5.4 update (Phase-5 T-38 / P19c — gate #40 SHIPPED real-disk; AC-28-49 promotes literal-cited → load-proven):** Created `linter-scripts/check-ci-cli-self-test-harness.py` (gate #40, 6 clauses, built-in `--self-test` with 6 fixtures F-1..F-6, exit codes `0`/`1`/`2`/`3`). Real-disk scan against `spec/28-universal-ci-cli/` passes 0 violations across all 6 modes. Bumped §04 v1.1.0 → **v1.2.0** declaring the cited `--self-test` global flag (with all 4 semantic markers `built-in`/`no network`/`no real CI provider`/`no real git repo`) plus `--check <mode>` enum (6 modes: `all` / `self-test-flag-declared` / `r5-vacuous-pass-carried` / `per-mode-fixture-coverage` / `exit-code-contract` / `harness-declaration`). Bumped §07 v1.1.1 → **v1.2.0** appending `## Self-test exit codes (gate #40 / --self-test)` 4-row table. Wired hard-fail step `§28 CI-CLI self-test harness gate (#40 / Phase-5 T-38 / P19c)` in `.github/workflows/spec-health.yml` directly after the §24 no-DDL gate. Lockstep: §97 v2.8.0 → **v2.9.0**; §00 v2.13.0 → **v2.14.0**; §98 [2.14.0] → **[2.15.0]**; this §99 v2.5.3 → **v2.5.4**; §04 v1.1.0 → **v1.2.0**; §07 v1.1.1 → **v1.2.0**. **One new wired gate (#40 promoted vapor → real); no new slot count change in §27 (slot 62 was already present); no AC count change (49 held).** Scorecard: §28 Lovable 120 → **120** (held); §28 Cursor 120 → **120** (held); §28 Raw-LLM 118 → **120** (+2 — C6 Friction lifts 18 → 20: gate is now reproducible from disk, previously vapor). **§28 reaches triple-ceiling L120/C120/R120 honestly.** Phantom-script ledger: 37 → **36**. Remaining P19 backlog: P19d (gate #42 `meta-verify-lockstep.py`), P19e (gate #39 `check-no-out-of-scope-spec-folder-link.py` + flip P19a from warn-only to hard).
